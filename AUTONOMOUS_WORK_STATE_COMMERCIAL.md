...
- Батч №4 = B1-fix-1 ГОТОВ (закрыты R-01..R-08, R-10, R-11, R-16, R-17 + «в квартал»): адаптер доставляет ess_min из bulk/tail + data_coverage из реальных дат + honesty_verdict/preflight; билдер: live-гейт is_live (wireframe-дефолты только в preview), _mstr «н/д»-рендер, честные «Данные и качество» (период/наблюдения/частота/разрывы из фактов), источники = файл клиента, CI-подписи 90% HDI (было 95%), n=200 (был 1000), приор без устаревшейσ, «12+ FMCG»/«90 дней»/«~12 мин» убраны из live. Канарейка tools/test_report_fidelity_live.py (6 тестов: запрет-лист 21 маркер + доставка честных значений + preview back-compat). Пере-зонд реального Kagocel: 0 запрет-маркеров, ESS 478 честный, «Вердикт надёжности: требует осторожности» + «Проверка приоров: расхождение (42%)» ДОСТАВЛЕНЫ. Gate: tools 1680 / sidecar 379 / narrative standalone 65/65. КОММИТ СЛЕДУЮЩИМ ШАГОМ.
- Остаток B1: fix-2 = R-09 (выдуманные рекомендации 15-20%), R-12 («32% продаж» без квалификатора), R-13 (SCQAR противоречие при lift=0/binding), R-14 («чёткая рекомендация» при Uncertain), R-15/R-18 (verify) → потом HTML-зонд тем же методом → xlsx.
...
| ID | Суть | Статус | Verify | Fix |
|---|---|---|---|---|
| B1 | Репортинг-канарейка: каждое ЧИСЛО отчёта (pptx/html/xlsx) ↔ JSON движка; каждый CLAIM ↔ фактический метод; honesty-поля доставлены | 🟡 PPTX-зонд СДЕЛАН → 18 находок (реестр B1-R ниже); фиксы батчами | зонд tmp/probe_b1_pptx_extract.py на реальном Kagocel (train→decompose→optimize→PPTX→извлечение всех текстов) + sources.json сверка + код-верификация каждой | — |
