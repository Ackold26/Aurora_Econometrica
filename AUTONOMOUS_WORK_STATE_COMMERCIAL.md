...
- Батч №3 = B1-зонд ЗАВЕРШЁН: 18 находок в реестре B1-R (верифицированы кодом). Незакоммиченных правок кода НЕТ. Следующий батч: B1-fix-1 (R-01,R-02,R-03,R-04,R-16 — честные данные в «Данные и качество» + honesty-доставка), затем B1-fix-2 (R-05..R-09,R-12..R-14 — источники/подписи/нарратив), затем канарейка-тест (запрет wireframe-маркеров в live-режиме) + HTML тем же зондом.
- Артефакты зонда: tmp/probe_b1_pptx_extract.py; PPTX+sources.json в %TEMP%/b1_pptx_xqqfnd86 (перегенерируемо).
