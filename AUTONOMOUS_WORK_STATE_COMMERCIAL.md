...
- Батч №9 = OPP-03 + OPP-08 ГОТОВ (A3 закрыта целиком): optimizer.py отдаёт `extrapolation` (per-period optimal vs p95/p99 истории, схема канала = F-01 goal-seek — единый язык); OptimizeStep forward-баннер (warn sev1 / danger sev2+, текст = goal-seek-карточка); compare: buildScenarioTails прокидывает extrapolation из scenario-JSON (шов резал поле!) + бейдж в таблице MultiScenarioPage с title-подсказкой + typedef. OPP-08 решение по месту: фронт не звал econ_forecast_scaling; потребность покрыта OPP-03 → endpoint остаётся внутренним API (математика масштабирования, ~10 тестов), Rust-мост задокументирован как кандидат E1 backtest (НЕ «фича с UI»). Тесты: test_optimizer_extrapolation.py (wide→sev2/tight→sev0). Gate: rust 165 / tools 1684 / sidecar 379 / svelte 0. КОММИТ СЛЕДУЮЩИМ ШАГОМ.
- Следующее по ПОРЯДКУ: B2 лицензии (стенд, матрица сценариев боем, #61 offline-smoke) — другая природа (Rust + стенд). Перед ним: живой gate Kagocel всего накопленного.
