...
- Батч №6 = B1-fix-3 (HTML) ГОТОВ: то же семейство закрыто в aurora_html — strings_ru.json (f1_leader/s04 «продаж»→«медиа-вклада», HalfNormal без 0.5, prior_note без «12+ FMCG», 95%→90% HDI, n=1000→200, cta без «90 дней») + sections.py (badge 90% HDI, «через 90 дней»→«после следующего периода», impact-card «+0 пп»→«Незначим (<0.5 пп)» без анимации, SCQAR-recommendation lift-гейт 0.5). HTML-слой оказался лучше PPTX: метрики условные (без числовых wireframe-дефолтов), uncertain-fallback уже был (SA19); ess_min/data_coverage доехали автоматически через общий адаптер. Канарейка: общая live-фикстура на оба формата + 4 HTML-маркера (8 тестов) — поймала живой «+0 пп» в impact-card. Gate: tools 1682 / sidecar 379 / канарейка 8/8 / verify-скрипты 35/35+43/43. КОММИТ СЛЕДУЮЩИМ ШАГОМ.
- Остаток B1: xlsx-экспорт (econ_export_xlsx — зонд числа↔JSON) + lib/scenario-export.js → закрыть B1. Потом A3.
