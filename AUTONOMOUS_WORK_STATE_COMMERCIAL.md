...
- Батч №13 = C2 ГОТОВ: межпроцессный file_lock боем (два процесса: LockTimeout 1.3с/русский текст/перезахват), OneDrive боем (build+load+validate+decompose ok), атомарность записи и idle-после-краша код-verify. Тест test_chaos_survival.py. Gate: tools 1697. КОММИТ СЛЕДУЮЩИМ ШАГОМ.
- Следующее по ПОРЯДКУ: C3 прицельный click-path по местам правок (нужен `npm run tauri:dev` + MCP-мост mcp__tauri__*; чек-лист: preflight-баннер, переключатель «Осторожный 80%», forward-extrapolation баннер, compare-бейдж, «Незначим <0.5пп», два окна GUI, вос-е после краша). Это ЖИВОЕ ОКНО — если мост недоступен в этой среде, зафиксировать пункты чек-листа в реестре и идти к D1.
