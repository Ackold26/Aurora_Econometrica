<script>
   ...
   *
   * 🔴 CPD-178 (проверено по коду 14.09.2026): запись реестра описывает механику версии 2.5.2,
   * где программа сама отправляла обращение — оттуда «двенадцать секунд отправки», очистка
   * поля и надпись на кнопке. В этой сборке того пути уже нет: `open_feedback_form` только
   * открывает форму в браузере, ничего не отправляя и не ожидая (см. комментарий выше и
   * `commands/feedback.rs`). Но сама болезнь СPD-178 была шире одной механики — успех кнопка
   * никак не подтверждала: `fbOpening` просто возвращался в `false`, и кнопка выглядела так же,
   * как до нажатия. Подтверждение стоит РЯДОМ с кнопкой, не на ней самой, и не гаснет по
   * таймеру — только когда человек начинает новую попытку (эталон переноса из Creative Hub).
   */
  let fbJustOpened = $state(false);
  let fbError = $state('');
  ...
  <main class="content">
    ...
    <section class="section">
      ...
      <div class="feedback-form">
        ...
        {#if fbOpening}
          <!-- Открытие браузера — короткое системное действие, не обращение к внешнему
               серверу: пояснять здесь нечего, надпись на кнопке уже честна. Признак живой
               работы даём отдельно, aria-live, чтобы не молчать дольше пары мгновений. -->
          <p class="fb-note" role="status" aria-live="polite">Открываю системный браузер...</p>
        {/if}
        {#if fbJustOpened}
          <!-- CPD-178: подтверждение успеха — рядом с формой, не на кнопке, и не гаснет
               само по таймеру (эталон Creative Hub, aurora-meta/CROSS_PRODUCT_DEFECT_REGISTRY.md). -->
          <p class="fb-success" role="status">
            Форма открылась в отдельном окне браузера. Заполните и отправьте её там – ответим
            в течение двух-трёх рабочих дней, срочный вопрос разберём в тот же день.
          </p>
        {/if}
        {#if fbError}
          <p class="fb-error" role="status">{fbError}</p>
          {#if fbErrorRaw}
    ...
    <section class="section">
      ...
      <p class="section-desc">Уникальный отпечаток этого компьютера. Передайте его администратору для привязки лицензии.</p>
      <div class="machine-id">
        <code>{machineId}</code>
      </div>
      <button class="copy-hash-btn" onclick={copyAllFingerprints}>
        {allCopied ? '✓ Скопировано!' : 'Скопировать отпечаток для лицензии'}
      </button>
    </section>
