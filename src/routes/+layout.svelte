<script>
  ...
  import SessionTimer from '$lib/components/SessionTimer.svelte';  // таймер сессии (countdown), сквозной на всех внутренних страницах
  ...
  // 🔴 CPD-85, корень закрыт 2026-08-14. Здесь стоял литерал версии, который правился
  // вручную при каждом выпуске — и однажды отстал на всю мажорную ветку (был '1.2.0' при
  // выпущенной 2.4.8). Сравнивается он с серверным порогом `app_min_version`: пока порог
  // 1.0.0, расхождение спит, но подними порог до 2.x — и КАЖДЫЙ клиент, включая только что
  // поставившего свежую сборку, решил бы, что устарел, и ушёл в требование обновиться.
  //
  // Теперь значение приходит из версии сборки: `display_version` отдаёт `CARGO_PKG_VERSION`
  // (`lib.rs:445`), то есть ровно то, чем собран этот exe, — расходиться больше нечему.
  // Тем же источником пользуется страница настроек, так что два места перестали жить
  // независимо друг от друга.
  let appVersion = '';
  invoke('display_version')
    .then((v) => { appVersion = String(v || ''); })
    .catch(() => { appVersion = ''; });
  ...
  onMount(() => {
    ...
    async function heartbeat() {
      try {
        ...
        // 🔴 Пустая `appVersion` (запрос версии сборки ещё не вернулся или отказал) —
        // это НЕ повод решать. `isNewer(порог, '')` разобрал бы пустую строку в нули и
        // объявил устаревшей любую сборку: ровно та ложная тревога, ради которой закрывался
        // CPD-85. Нет версии — пропускаем проверку до следующего сердцебиения.
        if (resp.app_min_version && appVersion && isNewer(resp.app_min_version, appVersion)) {
    ...
    (async () => {
      ...
      if (routeNotice) {
        // Признак «уведомление показано» снимаем НЕ здесь, а в момент закрытия
        // тоста (onClose ниже) — иначе он снимается сразу при вызове toast(), и
        // если программа закроется в первые секунды или тост не успеет
        // отрисоваться, сообщение о смене маршрута данных теряется навсегда.
        toast(routeNotice, 'info', 30000, acknowledgeRouteChange);
      }
    })();
  ...
  <div class="main-content">
    ...
    {#if $page.url.pathname !== '/'}
      <div class="global-topbar">
        <BrandChip />
        <div class="gt-timer"><SessionTimer /></div>
      </div>
    {/if}
...
<style>
  ...
  /* Таймер сессии — по центру топбара, независимо от ширины чипа (как на главной). */
  .gt-timer {
    position: absolute;
    left: 50%;
