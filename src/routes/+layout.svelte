<script>
  ...
  // 🔴 2026-08-14: константа отстала от продукта на всю мажорную ветку (была '1.2.0' при
  // выпущенной 2.4.8) и сравнивается с серверным порогом `app_min_version` в сердцебиении.
  // Пока порог на сервере 1.0.0, расхождение спит; поднимут порог до 2.x — и КАЖДЫЙ клиент,
  // включая только что установившего свежую сборку, решит, что устарел, и уйдёт в требование
  // обновиться. Корень — литерал вместо версии сборки; заведён отдельной записью реестра.
  // При смене версии продукта эту строку править ВРУЧНУЮ, пока корень не закрыт.
