 ...
 * с поведением. Пример: «≥ 85 – Отличное, 70–< 85 – Хорошее, … < 40 – Ненадёжное».
 *
 * Тире короткое (клиентский текст линейки). Нижняя граница уровня включительно,
 * поэтому верхняя граница диапазона помечена знаком «<» (исключая её саму) —
 * иначе «70–85» и «≥ 85» визуально пересекаются на 85, хотя по поведению
 * (`mqsTierInfo`) 85 уже уходит в «Отличное», а не в «Хорошее».
 *
 * @returns {string}
 */
export function mqsScaleText() {
  const parts = MQS_TIERS.map((t, i) => {
    const upper = i === 0 ? null : MQS_TIERS[i - 1].min;
    if (upper == null) return `≥ ${t.min} – ${t.label}`;
    if (t.min === 0) return `< ${upper} – ${t.label}`;
