    ...
    {#if result.distribution && Object.keys(result.distribution).length > 0}
      ...
      <section class="distribution">
        ...
        {#if result.allocation_mode === 'proportional'}
          <p class="dist-note">Бюджет масштабирован при текущих пропорциях каналов («Подбор под цель» отвечает «сколько нужно при нынешнем миксе»). Чтобы перераспределить между каналами, используйте прямой расчёт оптимизации.</p>
        {/if}
        <table>
          <thead>
            <tr>
