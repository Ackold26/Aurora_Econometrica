<script>
  ...
  const TASK_OPTIONS = [
    ...
    },
    {
      ...
      tooltip: 'Анализ «что если»: модель строит кривые отдачи в диапазоне бюджетов (базовый ±50%) и определяет точку маржинального ROI.',
      tone: 'info',
    },
    {
      ...
      tooltip: 'NEW v2.0.0: загрузите файл с плановыми активностями по каналам. Модель предскажет ожидаемый KPI с правдоподобными диапазонами.',
      tone: 'neutral',
    },
    {
      id: 'decompose-only',
  ...
  const crossProductWarning = $derived.by(() => {
    ...
    if (selected === 'decompose-only' && activeAdvertisingPct < 0.5) {
      return 'Декомпозицию запустить можно, но при малой доле периодов с рекламой ' +
             'правдоподобные диапазоны вкладов будут широкими.';
    }
    return null;
  });
