<script>
  ...
  const HELP = {
    ...
    roiCi: '95%-й правдоподобный диапазон ROI - диапазон, где истинный ROI лежит с вероятностью 95%.\n\nЧто это: [нижняя, верхняя] граница апостериорного распределения Bayesian-модели. Узкий интервал = высокая определённость, широкий = высокая неопределённость.\n\nПочему важно: показывает не точку, а РАЗБРОС оценки. Если диапазон = [0.8, 4.2] - мы плохо знаем ROI. Если диапазон = [2.1, 2.5] - оценка надёжна.',
  };
...
<div class="expert-panel">
  ...
  {#if paramRows.length > 0}
    <div class="section-title">Параметры каналов (апостериорные средние)</div>
      ...
      <table class="params-table">
        <thead>
          <tr>
            ...
            <th>95%-й диапазон<span class="help-icon" title={HELP.roiCi}>?</span></th>
          </tr>
        </thead>
        <tbody>
            ...
            <tr>
              ...
              <td class="mono" class:good={row.roi > 2} class:warn={row.roi < 1}>
                {#if row.roi != null && Number.isFinite(row.roi)}
                  {row.roi.toFixed(2)}x
                {:else}
                  <span class="placeholder-hint" title="ROI вычисляется на следующем шаге - Декомпозиции. Здесь показаны только апостериорные параметры сэмплера.">→ После декомпозиции</span>
                {/if}
              </td>
              <td class="mono ci">
                ...
                {:else}
                  <span class="placeholder-hint" title="Правдоподобный диапазон ROI вычисляется на шаге Декомпозиции из 8000 апостериорных сэмплов.">→ После декомпозиции</span>
                {/if}
              </td>
            </tr>
          {/each}
