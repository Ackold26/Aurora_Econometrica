<script>
  ...
  const HELP = {
    ...
    verdict:    'Вердикт - комбинированная оценка канала по ROI и Gap. Учитывает не только сырой ROI, но и эффективность относительно своей доли бюджета.\n\nСуффиксы:\n• «(широкий ROI-интервал)» - для канала Bayesian-CI шире точечного ROI. ROI следует читать как диапазон, а не точное число. НЕ означает что модель ненадёжна целиком - качество оценивается отдельно через R² / MAPE / R-hat (Эксперт-панель Train).\n\nТипичные причины широкого CI: мало наблюдений, низкая частота канала в данных, высокая корреляция с другими каналами.\n\n• «ROI завышен» - подозрение на смешанные единицы (канал не в рублях).',
  };
