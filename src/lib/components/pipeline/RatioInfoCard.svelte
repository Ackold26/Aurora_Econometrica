<script>
  ...
  const RATIO_HELP = [
    ...
    '• Идеальный ≥6:1 - узкие правдоподобные диапазоны, можно опираться',
...
<aside class="ratio-card tone-{statusMeta.tone}" aria-label="Соотношение данных к переменным">
  ...
  {#if expertMode}
    <div class="expert-block">
      ...
      {/if}
      <p class="expert-note">
        <strong>Thresholds:</strong> &lt;2:1 - overfit risk; 2-4:1 - модель работает, широкие диапазоны;
        4-6:1 - надёжно; ≥6:1 - узкие диапазоны, можно опираться на абсолютные значения.
      </p>
    </div>
  {/if}
</aside>
