    ...
    <div class="warn-banner warn">
      ...
      <details class="banner-details">
        ...
        <div class="banner-secondary">
          ...
          Рекомендуется увеличить число сэмплов или прогрев в расширенных настройках.
        </div>
      </details>
    </div>
  {/if}
  {#if divergences > 0}
    <div class="warn-banner warn">
      ...
      <details class="banner-details">
        <summary>Технические детали</summary>
        <div class="banner-secondary">
          Обнаружено: {divergences} {pluralizeRu(divergences, ['дивергенция', 'дивергенции', 'дивергенций'])} <span class="muted">({divergencesPct.toFixed(2)}% от {totalDraws} сэмплов)</span>
          <span class="muted">(Прогрев={mcmcTune}, Сэмплы={mcmcDraws}, target_accept={mcmcTargetAccept}).</span>
          {#if rhatFailed === 0}
            Параметры сошлись (R-hat &lt; 1.05) - модель готова к использованию.
            {#if divergences <= 10}
              {#if mcmcTune < 4000}
                <strong>Можно продолжать.</strong> Для академической чистоты - увеличьте прогрев (tune) до 4000-6000 в Эксперт-режиме.
              {:else if mcmcTargetAccept < 0.99}
                <strong>Можно продолжать.</strong> Прогрев уже {mcmcTune} - дальнейшее увеличение не поможет. 1-3 дивергенции практически безвредны для 95%-го правдоподобного диапазона.
              {:else}
                <strong>Можно продолжать.</strong> Прогрев={mcmcTune}, target_accept={mcmcTargetAccept} - настройки максимальные. Остаточные дивергенции говорят о геометрии апостериорного распределения, а не о том, что NUTS не адаптировался. Безопасно для 95%-го правдоподобного диапазона.
              {/if}
            {:else if divergences <= 50}
              {#if mcmcTune < 6000}
                <strong>Продолжать можно с осторожностью.</strong> Увеличьте прогрев до 6000 и число сэмплов (draws) до 4000 - обычно уменьшает дивергенции в 5-10 раз.
              {:else}
                <strong>Продолжать можно с осторожностью.</strong> Прогрев={mcmcTune} не помогает уменьшить дивергенции. Альтернативы: упростить модель (исключить коллинеарные каналы - см. VIF в Эксперт-режиме Валидации) или ужесточить приоры.
              {/if}
            {:else}
              {#if mcmcTune < 6000}
                <strong>Результаты использовать с осторожностью.</strong> Увеличьте прогрев до 6000+, число сэмплов до 6000, и/или исключите сильно коллинеарные каналы (см. VIF в Эксперт-режиме Валидации).
              {:else}
                <strong>Результаты использовать с осторожностью.</strong> Прогрев={mcmcTune} максимален - дальнейшее увеличение не поможет. Проблема в геометрии апостериорного распределения: упростите модель (уберите коллинеарные каналы по VIF), сократите количество параметров, или сузьте приоры.
              {/if}
            {/if}
          {:else}
            Параметры не сошлись (R-hat &gt; 1.05).
            {#if mcmcTune < 6000}
              Увеличьте прогрев до 6000 и число сэмплов до 4000; если не помогло - упростите модель (исключите коллинеарные каналы).
            {:else}
              Прогрев={mcmcTune} не помогает достичь сходимости. Упростите модель (уберите коллинеарные каналы по VIF) или пересмотрите приоры в Эксперт-режиме.
            {/if}
          {/if}
        </div>
      </details>
