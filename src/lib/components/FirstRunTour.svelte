  ...
  const steps = [
    {
      ...
      body: 'Это инструмент для маркетинг-микс моделирования. За 2 минуты покажу ключевые места - после этого сразу начнёшь работать. Готов?',
    },
    {
      selector: '[data-tour-step="import-file"]',
      title: 'Загрузка данных',
      body: 'Здесь загружается файл с историческими данными - Excel или CSV. Нужны хотя бы 20 недель по каналам и KPI. Перетащи файл или нажми для выбора.',
    },
    {
    ...
    },
    {
      ...
      body: 'Выбери что считать результатом рекламы: выручка в рублях, продажи в штуках, лиды. Это определяет в каких единицах модель покажет отдачу каналов.',
    },
    {
      selector: '[data-tour-step="run-model"]',
      title: 'Запуск модели',
      body: 'Нажми «Обучить модель» - Bayesian MCMC запустится на твоих данных. Займёт 1-5 минут. Пока идёт обучение, можно наблюдать за прогрессом.',
    },
    {
      selector: '[data-tour-step="results-section"]',
      title: 'Читаем результаты',
      body: 'Здесь ROI каждого канала, декомпозиция продаж на составляющие, качество модели. R² > 0.85 - хорошо. Кликни на канал чтобы увидеть кривую отклика.',
    },
    {
      selector: '[data-tour-step="scenario-playground"]',
      title: 'Сценарии «Что если»',
      body: 'Двигай слайдеры каналов - модель сразу покажет что изменится в продажах. Можно сохранить несколько сценариев и сравнить их между собой.',
    },
    {
      selector: null,
      title: 'Готово - начинай работать!',
      body: 'Тур завершён. Если понадобится подсказка - наводи мышь на названия метрик, появятся объяснения. Тур всегда можно перезапустить через Настройки.',
    },
  ];
