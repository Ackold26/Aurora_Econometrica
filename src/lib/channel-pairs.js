...
// B.1 (R2 2026-07-06): +контакт[а-яё]* — русская форма «Наружка Контакты» / «OOH Контакты».
// contacts? уже было (EN); кириллица отдельно, т.к. [а-яё] не покрывает EN и наоборот.
export const PHYSICAL_SUFFIX_RE = /(?:^|[_\s-])(?:impressions?|impr|clicks?|visits?|reach|contacts?|контакт[а-яё]*|grps?|trps?|показ[а-яё]*|клик[а-яё]*|визит[а-яё]*|охват[а-яё]*|просмотр[а-яё]*|грп[а-яё]*|трп[а-яё]*)(?=[_\s-]|$)/i;

/** Хвостовой суффикс метрики для среза базы канала (те же словоформы).
 * B.1 (R2 2026-07-06): +контакт[а-яё]* синхронно с PHYSICAL_SUFFIX_RE. */
const TAIL_METRIC_RE = /[_\s-](?:spend(?:s|ing)?|budget|cost(?:s)?|expense|бюджет[а-яё]*|расход[а-яё]*|затрат[а-яё]*|rub|usd|eur|impressions?|impr|clicks?|visits?|reach|contacts?|контакт[а-яё]*|grps?|trps?|показ[а-яё]*|клик[а-яё]*|визит[а-яё]*|охват[а-яё]*|просмотр[а-яё]*|грп[а-яё]*|трп[а-яё]*)$/i;
