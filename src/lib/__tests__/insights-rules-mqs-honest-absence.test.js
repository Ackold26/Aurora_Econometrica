...
describe('панель выводов: несчитанная оценка качества не подставляется нулём', () => {
  it('без метрики балл не показывается вовсе', () => {
    const text = textOf(modelInsights(dataWithoutMqs(), null));
  ...
  it('отсутствие оценки не выдаётся за низкое качество', () => {
    const insights = modelInsights(dataWithoutMqs(), null);
    const verdict = insights.find((i) => /MQS|оценка качества/i.test(i.text));
  ...
  it('реальный ноль остаётся видимым — это измерение, а не пропуск', () => {
    const text = textOf(modelInsights(dataWithZeroMqs(), null));
  ...
  it('регресс: посчитанный балл по-прежнему показывается с уровнем', () => {
    const high = textOf(modelInsights(dataWithScore(88, 'Отличное'), null));
    expect(high).toMatch(/MQS\s*=\s*88\s*\(Отличное\)/);

    const mid = textOf(modelInsights(dataWithScore(65, 'Приемлемое'), null));
