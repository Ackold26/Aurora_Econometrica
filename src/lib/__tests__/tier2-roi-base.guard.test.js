...
describe('roiBase — база выводится из выбранной метрики', () => {
  ...
  it('строковая маржа приводится к числу, а не роняет базу', () => {
    // Внешний аудит, High, 2026-08-02. Вызывающие передают сырое значение стора
    // мимо нормализации `kpiView`, а оно бывает строкой (собственный тест
    // продукта заведён на '120.5'). Проверка `typeof === 'number'` роняла такую
    // маржу в «не определена», тогда как движок при заданном kpi_unit_cost
    // пороги ПРИМЕНЯЕТ — экран и расчёт расходились на одном проекте.
    expect(roiBase({ kpiType: 'sales_packs', kpiKind: 'count', vpcu: '200' })).toBe('прибыль');
    expect(roiBase({ kpiType: 'sales_packs', kpiKind: 'count', vpcu: '120.5' })).toBe('прибыль');
  });

  it('нечисловая и неположительная маржа по-прежнему дают «не определена»', () => {
    expect(roiBase({ kpiKind: 'count', vpcu: 'не число' })).toBe('не определена');
    expect(roiBase({ kpiKind: 'count', vpcu: '-5' })).toBe('не определена');
    expect(roiBase({ kpiKind: 'count', vpcu: NaN })).toBe('не определена');
  });
...
describe('roiBaseNote — пояснение соответствует базе', () => {
  ...
  it('в режиме эффективности НЕ утверждает, что экономика не задана', () => {
    // Внешний аудит, Medium, 2026-08-02: у денежного проекта в режиме
    // эффективности экономика ЗАДАНА — отсутствуют абсолютные пороги, потому
    // что режим считает доли. Прежний общий текст врал ровно там, где блок
    // ложные утверждения и снимает.
    const note = roiBaseNote({ kpiType: 'sales', kpiKind: 'monetary', mode: 'effectiveness' });
    expect(note).not.toContain('Экономика не задана');
    expect(note).toContain('доли');
  });
});
...
describe('база доезжает в факты советника', () => {
  ...
  it('режим доезжает до советника: эффективность даёт «не определена»', () => {
    // Внешний аудит, High, 2026-08-02: `mode` не попадал в вызов `roiBase` из
    // контекста советника, и он называл базу «оборот» там, где экран
    // декомпозиции (он режим передаёт) честно говорил «не определена». Вход
    // единой функции обязан собираться одинаково во всех слоях — иначе «единый
    // источник» соблюдён формально, а результат расходится.
    expect(
      call({ kpiType: 'sales', kpiKind: 'monetary', derivedMode: 'effectiveness' })
        .facts.roi_base,
    ).toBe('не определена');
  });

  it('обычный режим по-прежнему даёт базу из метрики', () => {
    expect(
      call({ kpiType: 'sales', kpiKind: 'monetary', derivedMode: 'roi' }).facts.roi_base,
    ).toBe('оборот');
  });
