...
export function importInsights(data) {
  ...
  if (rows < 24) {
    out.push({ severity: 'warning', text: `Мало данных: ${rows} наблюдений (${period}). MMM требует минимум 2 года истории.`, tip: 'Байесовская модель работает с малыми выборками, но правдоподобные диапазоны будут широкими.' });
  } else if (rows >= 104) {
...
export function validateInsights(result, objective = 'roi', engine = 'bayesian', useHolidays = true, disabledHolidaysCount = 0) {
  ...
  if (maxMediaSum > 0) {
    ...
    if (outliers.length > 0) {
      ...
      out.push({
        ...
        tip: 'Байесовская модель нормализует все переменные перед обучением. Фактор в 100× больше остальных доминирует при нормализации, что приводит к нестабильности модели (R-hat > 1.5, высокие отклонения сэмплера) и переобучению. Рекомендуем: либо исправить данные в Excel, либо пометить колонку как «Не использовать».',
      });
    }
  }
  ...
  if (totalRows > 0 && mediaCols.length > 0) {
    ...
    if (ratio < 1) {
      ...
      out.push({
        severity: 'info',
        text: `Ratio ${ratio.toFixed(1)}:1 – достаточно для пилота. Для production рекомендуется ≥4:1, для более узких правдоподобных диапазонов ≥5:1.`,
      });
    } else {
      out.push({
        severity: 'success',
  ...
  if (result.correlations) {
    ...
    if (highCorr.length > 0) {
      ...
      out.push({ severity: 'warning', text: `Мультиколлинеарность: ${pairs}${highCorr.length > 3 ? ` и ещё ${highCorr.length - 3}` : ''}`, tip: 'Модель не сможет разделить вклады коррелирующих каналов. Решение: исключите один из пары, объедините в группу, или примите широкие правдоподобные диапазоны.' });
    }
...
export function modelPreTrainingInsights(validateResult, enabledMediaNames = undefined, engine = 'bayesian', useHolidays = true, disabledHolidaysCount = 0) {
  ...
  out.push({
    severity: 'info',
    text: 'Что происходит: модель оценит вклад каждого канала в KPI через Байесовскую регрессию с учётом остаточного эффекта (adstock) и насыщения (Hill).',
...
export function modelInsights(data, ratioOverride = undefined) {
  ...
  if (rSq < 0 || mape > 100) {
    ...
    out.push({
      ...
      tip: 'Действия: упростите модель (меньше каналов / контрольных), проверьте outlier-факторы в Валидации, либо переключитесь в режим OLS (Расширенные настройки → Эксперт) – он работает на малых выборках стабильнее байесовского.',
    });
  }

  // ── P0.6: отрицательный базовый уровень ──
  // Единственная проверка канона, за которой стоят деньги: она ловит завышенный
  // ROI. Если модель утверждает, что без рекламы продажи были бы отрицательными,
  // значит вклад медиаканалов приписан с избытком — и все ROI, и все советы по
  // бюджету завышены вместе с ним. Первоисточник класса дефекта: Wang & Jin,
  // «Hierarchical Bayesian… Media Mix with Category Data» — неограниченные приоры
  // дают содержательно невозможные оценки из-за пропущенных переменных.
  //
  // Показывается ДО предупреждения о тонких данных: тонкие данные делают выводы
  // широкими, а отрицательная база делает их неверными по знаку.
  //
  // Ключ приходит только у байесовских моделей (в OLS апостериорных выборок нет
  // вовсе). Его отсутствие — не «проверка пройдена», а «проверка недоступна»,
  // поэтому молчим, а не пишем успех.
  const negBase = d.negative_baseline;
  if (negBase && typeof negBase.prob_negative === 'number') {
    const вероятность = Math.round(negBase.prob_negative * 100);
    if (negBase.verdict === 'fail') {
      out.push({
        severity: 'error',
        text: `⚠ Базовый уровень продаж уходит в минус (вероятность ${вероятность}%). Модель фактически утверждает, что без рекламы продаж не было бы вовсе – это невозможно, значит вклад каналов завышен, а вместе с ним ROI и рекомендации по бюджету.`,
        tip: 'Причина обычно в пропущенном факторе, который двигал продажи вместе с рекламой: сезонный спрос категории, дистрибуция, промоакции, цена. Добавьте такой столбец как контрольный фактор и переобучите. Если добавить нечего – сократите число каналов: на коротком ряде модель приписывает рекламе то, что объяснить нечем.',
      });
    } else if (negBase.verdict === 'watch') {
      out.push({
        severity: 'warning',
        text: `Базовый уровень продаж близок к нулю (вероятность уйти в минус ${вероятность}%). Вклад каналов, скорее всего, завышен – трактуйте ROI как верхнюю границу, а не как точную оценку.`,
        tip: 'Проверьте, все ли внешние факторы учтены: сезонность категории, дистрибуция, цена, промо. Один пропущенный фактор, совпадающий с рекламной активностью, поднимает ROI всех каналов сразу.',
      });
    }
  }
  ...
  if (ratio > 0) {
    ...
    if (cls.severity === 'error') {
      ...
      out.push({
        ...
        tip: 'Байесовская модель с приорами (априорными ожиданиями) смягчает проблему, но не устраняет. Относитесь к декомпозиции как к ориентиру, а не истине. Для надёжности – ≥52 недель данных.' + ratioBridge,
      });
    }
  }
  ...
  if (mqs == null) {
    ...
    out.push({
      ...
      tip: thinnessCap
        ...
        : 'Можно работать, но добавление контрольных переменных (сезонность, праздники, промо) поднимет MQS и сузит правдоподобный диапазон.',
    });
  } else {
    out.push({
      severity: 'warning',
  ...
  if (rHat > 0 && rHat <= 1.01 && divergences === 0) {
    ...
    const convergenceTip = isThin
      ...
      : 'R-hat ≤ 1.01 означает, что независимые цепи сошлись к одному распределению. 0 дивергенций – сэмплер исследовал всё пространство параметров без скачков. Апостериорное распределение надёжно для оценки ROI и диапазонов значений.';
    out.push({
  ...
  if (nChannels > 0) {
    out.push({
      severity: 'info',
      text: `Структура модели: Байесовская модель с ${nChannels} канал${nChannels > 4 ? 'ами' : nChannels > 1 ? 'ами' : 'ом'} медиа. Каждый канал прошёл через остаточный эффект (adstock) + Hill saturation (убывающая отдача).`,
      tip: 'Остаточный эффект моделирует, что реклама прошлой недели продолжает работать сегодня. Hill – что после определённого порога каждый дополнительный рубль даёт меньше продаж. Это две стандартные нелинейности в эконометрике медиа.',
    });
  }
  ...
  if (isGoodModel) {
    out.push({
      ...
      tip: [
        ...
        '• Каждый ROI имеет 95%-й правдоподобный диапазон – видна неопределённость, не точечная оценка.',
        ...
        '• Sampler – NUTS (золотой стандарт байесовского вывода), не Metropolis.',
      ].join('\n'),
    });
  }
...
export function decomposeInsights(data, kpiInput = null) {
  ...
  if (basePct > 80) {
    out.push({
      ...
      tip: 'Возможные причины: (1) сильный бренд с лояльной аудиторией – медиа поддерживает, не двигает; (2) недостаточная мощность медиа-кампаний; (3) модель не уловила эффект (проверь остаточный эффект и контрольные переменные).',
    });
  } else if (basePct < 30) {
    out.push({
      severity: 'warning',
...
export function optimizeInsights(data, ctx = {}) {
  ...
  if (lift > 15) {
    ...
    out.push({
      severity: 'info',
      // 2026-09-08 (аудит Critical, шестая точка — найдена живым прогоном): прежний
      // текст утверждал «текущее распределение почти оптимально». Продукт этого не
      // знает: небольшой найденный прирост не доказывает близость к оптимуму —
      // зонд нашёл прямым счётом лучшее допустимое распределение там, где решатель
      // вернул ноль. Говорим то, что действительно известно: что нашёл решатель.
      text: `Прирост +${lift.toFixed(1)}% – решатель нашёл небольшое улучшение при заданных границах.`,
      tip: 'Потенциал в рамках того же бюджета невелик. Это результат поиска, а не доказательство оптимальности текущего распределения. Чтобы существенно улучшить – нужен либо рост бюджета (см. блок C What-if), либо пересмотр медиа-микса.',
    });
  } else {
    out.push({
      severity: 'warning',
      ...
      tip: kpi.isLegacy
        ? 'Это результат поиска, а не доказательство того, что лучшего распределения нет: решатель мог его не найти. Возможные причины: (1) большинство каналов уже на saturation plateau – каждый доп.рубль даёт меньше 1 рубля продаж, (2) Мин/Макс % слишком узкие, нет пространства для перекладки. Смотрите следующие инсайты для разбора по каналам.'
        : 'Это результат поиска, а не доказательство того, что лучшего распределения нет: решатель мог его не найти. Возможные причины: (1) большинство каналов уже на saturation plateau – каждый дополнительный рубль затрат почти не улучшает результат, (2) Мин/Макс % слишком узкие, нет пространства для перекладки. Смотрите следующие инсайты для разбора по каналам.',
    });
  }
  ...
  if (significantChanges.length > 0) {
    ...
    out.push({
      ...
      // 🔴 2026-09-08 (аудит Critical): прежний текст утверждал «подтверждение
      // того, что текущее распределение близко к оптимальному». Продукт этого не
      // знает: зонд на демо-наборе нашёл прямым счётом допустимое распределение
      // лучше текущего там, где решатель вернул нулевой прирост. Говорим факт
      // поиска, а не вывод об оптимальности — иначе панель подсказок противоречит
      // баннеру шага «Оптимизация».
      tip: 'Решатель не нашёл заметно лучшего распределения в рамках заданных лимитов. Это результат поиска, а не доказательство оптимальности текущего распределения.',
    });
  }
  ...
  if (noLift) {
    ...
    if (manyChannelsSaturated) {
      // 2026-09-08 (аудит Critical, решение ведущей): прежний текст обещал, что
      // оптимизатор «получит пространство для перекладки». Действие поднимается при
      // нулевом приросте — ровно в том случае, который замерен как нечувствительный
      // к ширине коридора: 20/200, 10/300 и 0/500 дали один и тот же ноль. Обещание
      // убрано, вместо него клиенту дана трактовка исхода, чтобы он не ушёл в тупик.
...
export function planningInsights(ctx = {}) {
  ...
  if (!(nFuture > 0)) {
    // Находка 3 внешнего аудита (Medium, 2026-09-13): `restoreProjectResults`
    // ставит `mediaPlanProbeStatus` в `null` не только когда поиск плана не
    // смог определить исход (`detection: 'unavailable'`), но и когда в
    // results/*.json секции медиаплана вовсе нет (project-state.js:1353) –
    // проект создан до появления поиска плана, либо валидация его не
    // сохраняла. `null` в обоих случаях означает «проверка не выполнена или
    // её исход неизвестен», а НЕ «проверили и строк на будущее нет» – это
    // разные факты, и печатать утвердительную ветку при `null` было прямым
    // нарушением INV-50 (утверждение о данных без выполненной проверки).
    const unavailable = probeStatus === 'unavailable' || probeStatus === null;
    out.push({
  ...
  if (comparable.length >= 2) {
    ...
    // P8/P9 (находка 2 внешнего аудита, 2026-09-13): раньше P8 считал перекрытие
    // по ВСЕМ парам сравнимых линий, а P9 – только по паре «лидер vs ближайший
    // преследователь» (sorted[1]). Разные выборки могли одновременно дать
    // противоречивые выводы об одном и том же лидере (пример из аудита: три
    // линии, у P8 2 из 3 пар перекрываются → «преимущество не доказано», у P9
    // пара «лидер–второй» не перекрывается → «лидер устойчиво лучше» – два
    // взаимоисключающих вывода в одной панели одновременно).
    // Единственная выборка – пара «лидер vs ближайший преследователь по
    // центральной оценке» (это уже была выборка P9): она отвечает на прямой
    // вопрос шага «можно ли доверять текущему лидеру», и third-party варианты
    // не размывают вывод про лидера, в отличие от all-pairs. P8 и P9 теперь
    // взаимоисключающие по построению – одна и та же пара либо перекрывается
    // (P8), либо нет (P9), третьего не дано.
    const overlap = first.lo < second.hi && second.lo < first.hi;
    if (overlap) {
      // P8. НЕ появится: сравнимых линий меньше двух либо диапазоны не пересекаются.
      out.push({
        severity: 'warning',
        text: `Диапазон лидера «${first.name}» (${fmt(first.kpi)}) пересекается с диапазоном ближайшего преследователя «${second.name}» (${fmt(second.kpi)}) – преимущество лидера «${first.name}» данными не доказано. Выбирайте по цене исполнения и рискам, а не по разнице прогнозов; либо разведите планы сильнее.`,
    ...
    // Знаковый граничный случай (first.lo === second.hi ровно) не перекрытие
    // (overlap строго <) и не «устойчиво лучше» (P9 строго >) – молчание на
    // самой границе намеренно, как и раньше у P9 в одиночку.
  }
...
export function reportInsights(ctx = {}) {
  ...
  if (reportRefused) {
    ...
    out.push({
      ...
      tip: `Рекомендация: используйте выводы отчёта для бюджетных решений. Спецификация модели, priors и правдоподобные диапазоны экспортируются в PPTX/MD/XLSX для воспроизводимости.\n\nВалидационный критерий: посмотрите график «Факт vs Прогноз» в отчёте – линии должны накладываться без систематического смещения.`,
    });
  } else if (isThin) {
    out.push({
      severity: 'warning',
