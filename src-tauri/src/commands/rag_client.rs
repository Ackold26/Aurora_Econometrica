...
pub async fn econ_rag_search(
    ...
    #[cfg(feature = "cloud_advisors")]
    {
        ...
        // 🔴 Б-50 (находка внешнего аудита): гейты выше проверяют выбор ЧЕЛОВЕКА (левое
        // положение, согласие) — но не проверяли, входит ли в САМУ СБОРКУ путь к шлюзу
        // Авроры. Без него переключатель показан заблокированным слева с подписью
        // «материалы не покидают эту машину», а при снятом local_only и данном согласии
        // обращение к библиотеке методологии было формально возможно — подпись и
        // поведение расходились. Путь сегодня мёртв (нужен `AURORA_RAG_SECRET`, которого
        // нет ни в одной сборке), но расхождение подписи и поведения обязано быть закрыто
        // само по себе, а не только отсутствием секрета. Проверка стоит ПОСЛЕДНЕЙ, тем же
        // порядком, что и `claude.rs::ensure_gateway_route` (решение владельца
        // 10.09.2026): сначала — можно ли обращаться наружу вообще и есть ли согласие
        // (выбор человека), и лишь потом — умеет ли ЭТА сборка физически дотянуться до
        // шлюза. Один и тот же трёхзвенный гейт обязан идти в одном и том же порядке
        // везде, иначе расходящийся порядок сам становится источником путаницы.
        if !crate::commands::execution_mode::cloud_built_in() {
            return Err("[RAG-NO-GATEWAY] В эту сборку не входит путь к шлюзу Авроры – библиотека методологии недоступна.".to_string());
        }
