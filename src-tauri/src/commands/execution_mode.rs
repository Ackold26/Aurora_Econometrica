...
/// То же сообщение для того, кто после переноса оказался СЛЕВА: согласие отозвано или дано
/// на прежнюю версию условий. Путь через свой Claude Code закрыт, а шлюз ещё не включён.
pub const ROUTE_CHANGED_NOTICE_LEFT: &str = "Маршрут ИИ-ассистента изменился. Раньше запросы \
    шли в программу Claude Code, установленную на вашем компьютере; этот путь больше не \
    поддерживается – ассистент работает только через шлюз Авроры. Сейчас ассистент отключён, \
    материалы не покидают эту машину. Чтобы задавать вопросы Авроре, включите работу через \
    шлюз в Настройках.";

/// То же сообщение для сборки, в которой переключатель заблокирован слева: включить шлюз
/// нельзя, и звать человека в Настройки было бы обещанием, которого сборка не выполнит.
pub const ROUTE_CHANGED_NOTICE_LOCKED: &str = "Маршрут ИИ-ассистента изменился. Раньше запросы \
    шли в программу Claude Code, установленную на вашем компьютере; этот путь больше не \
    поддерживается. В этой сборке ассистенту обращаться некуда, поэтому он отключён, а \
    материалы не покидают эту машину.";

/// Какое сообщение о смене маршрута показать — по ФАКТИЧЕСКОМУ положению в момент показа.
///
/// 🔴 Находка внешнего аудита 11.09.2026: перенос ставил признак сообщения, не спросив
/// согласия, а единственный текст утверждал «теперь они идут через шлюз Авроры». У кого
...
///
/// 🔴 Находка внешнего аудита 13.09.2026: обращение в форму на деле уносит СЕМЬ полей
/// (`feedback.rs::feedback_form_link`), а подпись называла только четыре — версия Windows,
/// шаг мастера и текст последней ошибки были умолчаны. Дописаны все три; про шаг мастера и
/// текст ошибки сказано явно, что они едут ТОЛЬКО при нажатии «Сообщить о проблеме» в окне
/// ошибки — из настроек этих двух полей программа взять не может, там их и не подставляет.
pub const HEADLINE_LOCAL: &str = "Сейчас: материалы не покидают эту машину – ИИ-ассистент \
    ...
    содержимого кабинетов и оболочки. Отдельно: форма обращения в поддержку открывается в \
    браузере по вашему нажатию – в неё программа подставляет название, версию, отпечаток \
    машины, время и версию Windows, а при нажатии «Сообщить о проблеме» в окне ошибки – ещё \
    и шаг мастера с текстом ошибки.";
...
mod tests {
    ...
    /// 🔴 Сообщение о смене маршрута не утверждает маршрута, которого нет.
    ///
    /// Перебираются ВСЕ сочетания правила положения (четыре слагаемых), а не три
    /// разобранных случая: ошибаются в пропущенном сочетании. Для каждого текст сверяется
    /// с тем же `route_is_cloud`, по которому считается подпись «Сейчас: …».
    #[test]
    fn route_changed_notice_never_claims_a_route_that_is_not_in_force() {
        for mask in 0u8..16 {
            let local_only = mask & 1 != 0;
            ...
            assert!(text.contains("Claude Code"), "что было: {why}");
            assert!(!text.contains('—'), "клиентский текст — короткое тире: {why}");
            if cloud {
                assert!(text.contains("теперь они идут через шлюз"), "что стало: {why}");
            } else {
                assert!(
                    !text.contains("теперь они идут"),
                    "слева нельзя утверждать, что запросы идут через шлюз: {why}",
                );
                assert!(text.contains("не покидают эту машину"), "что сейчас: {why}");
                assert_eq!(
                    text.contains("Настройках"),
                    !locked,
                    "в Настройки зовём только туда, где переключатель можно сдвинуть: {why}",
                );
            }
        }
    }
