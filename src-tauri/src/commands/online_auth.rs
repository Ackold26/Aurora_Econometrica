...
mod tests {
    ...
    /// Зонд живой подписи: собирает НАСТОЯЩИЙ запрос производственным путём (`собрать_запрос`)
    /// и печатает готовые заголовки с телом — чтобы отправить их в боевую дверь входа.
    ///
    /// 🔴 Почему именно так, а не переписью подписи на другом языке: перепись доказывала бы
    /// согласие переписи с самой собой, а нам нужно согласие КЛИЕНТА с СЕРВЕРОМ. Отсюда правило:
    /// для кода, обязанного совпасть с чужой реализацией байт в байт, приёмка — прогон чужой
    /// стороной, а не свои тесты, сколько бы их ни было.
    ///
    /// Отпечаток синтетический: лицензии под ним нет и быть не должно, поэтому вход ответит
    /// «лицензия не найдена» — и запишет в журнал вердикт подписи, что зонду и нужно.
    ///
    /// Запуск: `cargo test --lib зонд_живой_подписи -- --ignored --nocapture`
    #[test]
    #[ignore = "печатает готовую подпись для отправки в бой; запускать вручную"]
    fn зонд_живой_подписи() {
        use crate::crypto::request_sig as подпись;
        ...
        let mut req = образец_запроса();
        req.fingerprint_hash = "ab".repeat(32);
        req.hostname = "probe-sign-0906".to_string();
        let байты = serde_json::to_vec(&req).expect("тело сериализуется");

        let адрес = "https://quzhkfvglqmppxcrindh.supabase.co/functions/v1/auth";
        let запрос = собрать_запрос(&client, адрес, &req, Some(&байты), Some(&ключ))
            .build()
            .expect("запрос обязан собираться");

        println!("ЗОНД_URL={}", запрос.url());
        for имя in [
            подпись::HEADER_DEVICE,
            подпись::HEADER_TIMESTAMP,
            подпись::HEADER_NONCE,
            ...
            println!(
                "ЗОНД_{}={}",
                имя,
                значение_заголовка(&запрос, имя).expect("заголовок подписи на месте")
            );
        }
        println!("ЗОНД_BODY={}", String::from_utf8(байты).expect("тело — UTF-8"));
        println!("ЗОНД_ОТПЕЧАТОК={}", req.fingerprint_hash);
        let _ = std::fs::remove_dir_all(&dir);
    }
