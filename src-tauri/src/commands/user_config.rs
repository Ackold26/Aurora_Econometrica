...
/// правки + двузначный номер редакции ЭТОГО дня. Формат выбран вместо числа осознанно
/// (компромисс 2 передачи блока s48, `Projects/handoff.md`): переход на число потребовал бы
/// миграции сохранённых отметок согласия у уже установленных копий. Монотонность при
/// строковом сравнении — часть того же соглашения (инвариант 5), и её держит двузначность
/// номера.
///
...
/// однозначной записи `".10" < ".2"` лексикографически (десятая правка читается как меньшая
/// второй) — та же ловушка на новом уровне.
///
/// 🔴 s49, 18.09.2026 (находка 5 внешнего аудита): `trial_consent_outdated` ниже сравнивает
/// теперь на НЕСОВПАДЕНИЕ, а не на «строго меньше». Требование двузначности этим не
/// отменяется: оно держит монотонность формата (инвариант 5) на будущее и единообразие
/// записи номера в константе и в юридически согласованном тексте чекбокса.
///
...
///
/// 🔴 Сравнение на НЕСОВПАДЕНИЕ, а не на «строго меньше» (находка 5 внешнего аудита,
/// s49 18.09.2026). Прежнее `<` отвечало на вопрос «старее ли», а требуется «та ли самая»:
/// согласие на БОЛЕЕ НОВУЮ редакцию считалось действительным для старой сборки. Сценарий –
/// человек принял условия в будущей версии программы и поставил поверх неё нынешнюю; папка
/// настроек общая (`%APPDATA%`), конфиг уцелел, окно не поднималось, и он работал под
/// документом, которого в ЭТОЙ сборке нет.
///
/// Сравнение осталось строковым намеренно: формат `ГГГГ-ММ-ДД.НН` выбран вместо числа
/// именно потому, что переход на число потребовал бы миграции сохранённых отметок согласия
/// у уже установленных копий (осознанный компромисс 2 передачи блока s48,
/// `Projects/handoff.md`). Правка на равенство миграции не требует – она меняет только
/// направление вопроса.
...
mod tests {
    ...
    /// 🔴 Находка 5 внешнего аудита (s49, 18.09.2026), первая половина: согласие на БОЛЕЕ
    /// НОВУЮ редакцию считалось действительным для старой сборки.
    ///
    /// Сценарий: человек принял условия в будущей версии программы, поставил поверх неё
    /// нынешнюю (папка настроек общая, `%APPDATA%`, конфиг уцелел) – окно не поднималось, и
    /// он работал под документом, которого в ЭТОЙ сборке нет. Сравнение «строго меньше»
    /// отвечало на вопрос «старее ли», а требовался вопрос «та ли самая».
    #[test]
    fn trial_consent_from_a_newer_revision_still_shows_the_window() {
        let from_the_future = Some(TrialConsent {
            revision: "2099-10-01.01".to_string(),
            accepted_at: 1,
        });
        assert!(
            trial_consent_outdated(&from_the_future),
            "согласие на редакцию из будущей сборки не должно закрывать показ окна: в ЭТОЙ \
             сборке лежит TRIAL_TERMS_REVISION = {TRIAL_TERMS_REVISION}, и человек обязан \
             увидеть именно её текст"
        );
    }

    /// 🔴 Находка 5, полный перечень: окно поднимает ЛЮБОЕ несовпадение сохранённой
    /// редакции с редакцией сборки – и старее, и новее.
    ///
    /// Перечень строится по нескольким признакам сразу: старее, новее в пределах дня, новее
    /// по дате, прежний формат голой даты, пустая запись, мусор, и – отдельной строкой –
    /// совпадение, без которого перечень доказывал бы лишь то, что окно поднимается всегда.
    ///
    /// Ось мутации: вернуть `<` – краснеет на любой строке «новее»; заменить на `==` –
    /// краснеет на совпадении.
    ///
    /// Чего этот тест НЕ проверяет: беду «.10 против .3». Она закрыта не здесь, а
    /// соглашением о формате – двузначностью номера (инвариант 5 и компромисс 2 передачи
    /// блока s48). За двузначность отвечает отдельный сторож
    /// `trial_terms_revision_matches_frontend_checkbox_text`; он проверен мутацией константы
    /// 18.09.2026 и краснеет на однозначном номере – подробности в его комментарии.
    #[test]
    fn any_mismatch_with_the_build_revision_shows_the_window() {
        let cases: &[(&str, bool, &str)] = &[
            (TRIAL_TERMS_REVISION, false, "та же редакция – окно не поднимаем"),
            ("2020-01-01.01", true, "редакция старее – поднимаем"),
            ("2099-10-01.01", true, "редакция новее по дате – поднимаем"),
            ("2026-09-14.99", true, "тот же день, другой номер – поднимаем"),
            ("2026-09-14", true, "прежний формат голой даты – поднимаем"),
            ("", true, "пустая запись – поднимаем"),
            ("совсем-не-редакция", true, "мусор – поднимаем"),
        ];
        for (saved, expected, why) in cases {
            let consent = Some(TrialConsent { revision: (*saved).to_string(), accepted_at: 1 });
            assert_eq!(
                trial_consent_outdated(&consent),
                *expected,
                "{why}: сохранено {saved:?}, в сборке {TRIAL_TERMS_REVISION:?}"
            );
        }
    }
}
...
mod trial_terms_frontend_consistency_guard {
    ...
    fn trial_terms_revision_matches_frontend_checkbox_text() {
        ...
        // сравнение (".10" < ".2" лексикографически) даёт ту же ловушку монотонности, что
        // уже чинили на уровне даты; монотонность формата закреплена инвариантом 5 передачи
        // блока. u32::parse однозначный номер разбирает без возражений, поэтому длину
        // проверяем отдельно.
        //
        // 🔴 s49 (18.09.2026): сторож проверен мутацией константы, а не на слово. Однозначный
        // номер ("2026-09-14.2") краснит проверку длины; чужая дата ("2026-09-15.02") краснит
        // сверку даты с текстом чекбокса; чужой номер ("2026-09-14.03") краснит сверку номера.
        // Все три места живые, оба текста (константа и текст чекбокса) действительно сверяются.
        assert_eq!(
            number_part.len(),
            2,
            "номер редакции в TRIAL_TERMS_REVISION обязан быть двузначным (docstring требует \
             ГГГГ-ММ-ДД.НН) - иначе при строковом сравнении \".10\" < \".2\", и монотонность \
             формата (инвариант 5 передачи блока) ломается на новом уровне; получено \
             '{number_part}' (TRIAL_TERMS_REVISION = {TRIAL_TERMS_REVISION})"
        );
