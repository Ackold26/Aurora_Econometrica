...
/// 🔴 `replace: true` обязателен (находка внешнего аудита). Приёмник добавляет
/// `result` в чат ТОЛЬКО когда последнее сообщение не от ассистента: на локальном
/// пути это верно — там `result` дублирует уже показанное. Облачный путь дописывает
/// в финальный текст то, чего в потоке не было (номер осиротевшей работы,
/// «Ответ неполный», не доехавшие файлы выгрузки), и без этого признака весь
/// дописанный хвост молча выбрасывался. Признак ставит только облачный путь —
/// в потоке локального его нет, поведение локального пути не меняется.
...
async fn execute(
    ...
    let state = match outcome {
        ...
        Err(e) => {
            ...
            if !suppress_export {
                ...
                // связь оборвалась, в отчёт легла половина работы. Пометка в теле
                // текста есть, но отбирают такие файлы именно по имени — через
                // неделю усечённый разбор уйдёт клиенту как законченный.
                auto_save_response(&app_handle, work_dir, &cabinet_id, prompt, &text, true);
            }
    ...
    if !suppress_export {
        // 🔴 Усечённый по времени ответ помечается и в ИМЕНИ файла — `…-partial.md`,
        // как на локальном пути (находка внешнего аудита). Прежде облачный путь
        // всегда писал имя законченного отчёта: половина работы лежала в папке
        // неотличимо от полной, и при разборе через неделю уходила клиенту.
        auto_save_response(
            &app_handle, work_dir, &cabinet_id, prompt, &text, state.status == "timeout",
        );
    } else {
...
mod tests {
    ...
    fn result_event_parses_back_with_type_and_text() {
        ...
        // Без признака замены приёмник выбрасывает финальный текст со всеми
        // приписками, если поток уже показал хоть кусок (находка внешнего аудита).
        assert_eq!(v["replace"], true, "финальное событие обязано требовать замены показанного");
    }
    ...
    /// 🔴 Адрес узла не был закрыт ни одной проверкой (приёмка 17.08). Это
    /// единственное место, решающее, КУДА уезжают материалы клиента: подмени
    /// строку — задания пойдут на чужой вход, а все прочие проверки останутся
    /// зелёными, потому что ни одна из них адреса не касается. Отдельно
    /// проверяется защищённость входа: переход на `http` сделал бы билет доступа
    /// и материалы читаемыми по дороге.
    ///
    /// Переопределение стендом обязано работать — на нём проверяют облачный путь
    /// без боевого узла, — но умолчание остаётся нашим.
    #[test]
    fn node_address_defaults_to_our_gateway_and_yields_to_the_stand() {
        let saved = std::env::var("AURORA_CLOUD_URL").ok();
        std::env::remove_var("AURORA_CLOUD_URL");
        let default_url = gateway_base_url();
        assert_eq!(
            default_url, "https://rag.auroraai.pro/cloud",
            "материалы клиента уедут не на наш вход: {default_url}",
        );

        std::env::set_var("AURORA_CLOUD_URL", "http://127.0.0.1:8787/cloud");
        assert_eq!(
            gateway_base_url(),
            "http://127.0.0.1:8787/cloud",
            "стенд обязан перебивать умолчание, иначе облачный путь негде проверить",
        );

        match saved {
            Some(value) => std::env::set_var("AURORA_CLOUD_URL", value),
            None => std::env::remove_var("AURORA_CLOUD_URL"),
        }
    }

    /// 🔴 Ключ отправки обязан РАЗЛИЧАТЬСЯ между отправками (приёмка 17.08).
    /// Он собирается как `{метка диалога}-{короткая отметка}` и делает повтор
    /// безопасным: по одному и тому же ключу сервер отдаёт прежнее задание вместо
    /// нового. Стань отметка постоянной — второй вопрос в том же диалоге молча
    /// вернул бы ответ на первый, и заметить это можно только по содержанию
    /// ответа. Проверка была невозможна вовсе: модуль не участвовал в приёмке.
    ///
    /// Берём набор вызовов, а не два подряд: отметка идёт от часов, и требование
    /// «два соседних значения различны» зависело бы от разрешения таймера машины.
    #[test]
    fn submit_key_stamp_changes_between_sends() {
        let stamps: std::collections::HashSet<String> =
            (0..1000).map(|_| short_stamp()).collect();
        assert!(
            stamps.len() > 1,
            "отметка ключа отправки постоянна — сервер вернёт прежнее задание вместо нового",
        );
        for stamp in &stamps {
            assert!(
                !stamp.is_empty() && stamp.len() <= 8 && stamp.chars().all(|c| c.is_ascii_hexdigit()),
                "отметка обязана быть короткой шестнадцатеричной: {stamp}",
            );
        }
    }
}
