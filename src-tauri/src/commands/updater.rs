...
pub fn verify_checksum(file_path: &std::path::Path, expected: &str) -> Result<()> {
    if expected.is_empty() {
        return Err(user_err(
            ErrorCode::UP003,
            "Обновление опубликовано без контрольной суммы, поэтому проверить его целостность нельзя – непроверенный файл программа не установит. Повторная попытка тут не поможет: напишите нам на sales@auroraai.pro, и мы поправим это на своей стороне.",
        ));
    }
...
mod tests {
    ...
    fn checksum_empty_refused() {
        ...
        assert!(msg.contains("sales@auroraai.pro"), "человеку нужен адрес, куда писать: {msg}");
        let _ = std::fs::remove_file(&p);
    }
