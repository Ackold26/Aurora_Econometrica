...
///
/// Границы не пересекаются: верхняя граница диапазона явно исключена
/// (`X ≤ MQS < Y`), в согласии с поведением «нижняя граница включительно» у
/// `mqs_tier_for_score`. Нотация `≤`/`<` для полуоткрытых диапазонов в
/// клиентском тексте уже прецедентна в кодовой базе (см.
/// `src/lib/insights-rules.js`, `src/lib/ratio-classifier.js`).
pub fn mqs_scale_text() -> String {
    let mut parts: Vec<String> = Vec::with_capacity(MQS_TIERS.len());
    for (i, t) in MQS_TIERS.iter().enumerate() {
        if i == 0 {
            parts.push(format!("MQS \u{2265} {:.0} \u{2013} {}", t.min, t.label));
        } else {
            let upper = MQS_TIERS[i - 1].min;
            if t.min == 0.0 {
                parts.push(format!("MQS < {upper:.0} \u{2013} {}", t.label));
            } else {
                parts.push(format!(
                    "{:.0} \u{2264} MQS < {upper:.0} \u{2013} {}",
                    t.min, t.label
                ));
            }
        }
...
mod tests {
    ...
    fn scale_text_covers_all_five_tiers_without_overlap() {
        ...
        assert!(s.contains("\u{2265} 85"));
        assert!(s.contains("< 40"));
        // Границы не пересекаются: 85 не встречается как верхняя граница
        // диапазона "Хорошее" (было бы "70 ≤ MQS < 85" рядом с "≥ 85" -
        // формально не пересечение множеств, но и не голое "70–85").
        assert!(s.contains("70 \u{2264} MQS < 85"));
        assert!(s.contains("55 \u{2264} MQS < 70"));
        assert!(s.contains("40 \u{2264} MQS < 55"));
