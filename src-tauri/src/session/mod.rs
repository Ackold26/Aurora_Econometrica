...
mod tests {
    ...
    /// 🔴 Сторож СВЯЗИ (тот же урок Ф-04, что и у раскладки выдач): модуль `win_acl` покрыт
    /// собственными тестами, но починка прав существует ТОЛЬКО пока её кто-то зовёт. Если вызов
    /// из `SessionManager::new()` уберут — все тесты `win_acl` останутся зелёными, а каталоги
    /// клиентов, испорченные прежними версиями, снова перестанут чиниться, и никто этого не
    /// заметит. Проверка идёт по `code_only`: упоминание в комментарии сторожа не удовлетворяет
    /// (ровно та дыра, которую мутация приёмщика нашла у соседнего сторожа 29.07.2026).
    #[test]
    fn session_manager_still_repairs_directory_permissions_on_start() {
        let manager_src = code_only(&read_src("manager.rs"));

        assert!(
            manager_src.contains("win_acl::restore_inheritance"),
            "session/manager.rs обязан звать win_acl::restore_inheritance при инициализации — \
             иначе каталог сессий, у которого прежние версии сняли наследование прав, так и \
             останется без доступа SYSTEM и Администраторов (хвост CPD-77)"
        );
    }
}
