...
const ALLOWED_INFRA: &[(&str, &str)] = &[
    ...
    // 🔴 Четвёртый путь наружу, найден этим сторожем 13.09.2026 при подключении обращения к форме.
    // Отличается от трёх выше: программа сама туда НЕ ходит – она открывает адрес в браузере по
    // нажатию человека, а сведения (название, версия, отпечаток, время) едут предзаполнением в
    // самой ссылке. Для человека разница невелика: материалы остаются на машине, а сведения о
    // программе уходят – поэтому обращение названо в подписи наравне с остальными, а не умолчано
    // под предлогом «это не наш запрос».
    ("commands/feedback.rs", "обращени"),
];
