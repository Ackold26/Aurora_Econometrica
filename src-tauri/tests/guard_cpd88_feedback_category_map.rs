...
fn extract_settings_category_values(markup: &str) -> Vec<String> {
    ...
    let значения: Vec<String> = block
        .split("<option value=\"")
        .skip(1)
        .map(|chunk| chunk.split('"').next().unwrap_or_default().to_string())
        .collect();

    // 🔴 Внешний аудит блока 2.4.10: разбор принимал ЧАСТИЧНЫЙ результат за полный.
    // Разбиение идёт по буквальному `<option value="`, и любая иная форма записи —
    // `<option class="x" value="…">`, `<option selected value="…">`, одинарные кавычки —
    // просто не извлекается. Тема, записанная так, не проверяется ни этим тестом (её нет
    // в списке), ни проверкой сирот (пары в карте тоже нет) — оба сторожа зелены, а форма
    // отвечает отказом на живую тему. Поэтому число извлечённых значений обязано совпадать
    // с числом самих `<option` в блоке: разбор, который что-то не понял, — это отказ.
    let всего_вариантов = block.matches("<option").count();
    assert_eq!(
        значения.len(),
        всего_вариантов,
        "разбор переключателя тем неполон: вариантов в разметке {всего_вариантов}, извлечено \
         {}. Значит какая-то тема записана иначе, чем ожидает сторож (атрибут перед value, \
         одинарные кавычки, перенос строки) — она не проверяется, и регресс CPD-88 пройдёт \
         мимо зелёного сторожа. Чинить разбор, а не эту проверку.",
        значения.len()
    );
