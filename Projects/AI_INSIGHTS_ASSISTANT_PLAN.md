...
**Текущее состояние: Аврора (Фазы 0–3) + справка + методология + live-проверка — ВСЁ ЗАПУШЕНО. UX-фикс блока «Что если» (STEP.OPTIMIZE, `1555a8d`) запушен. ИДЁТ: синхронизация ОТЧЁТНОСТИ (HTML/PPTX/XLSX) с программой — Волна 1 честность/INV-50. Шаг 1 (единый honesty-тон) ЗАПУШЕН `072a6ca`. Шаг 2 (заглушка битых ROI) ядро ЛОКАЛЬНО `18d47d4` (НЕ запушен). Карта остатка — секция «Аудит отчётности» ниже.**
...
**Сделано Волна 1:** ✅ Шаг 1 `072a6ca`(запушен) — единый honesty-тон (`diagnostics.format_thinness_caveat`+verdict→McElreath, чинит HTML+PPTX+UI; `report.rs` Rust-зеркало синхрон; `_merge_channels` прокидывает unit_smell). Тесты 13/13, cargo 0, на реальном прогоне. 🟡 Шаг 2 `18d47d4`(локально) — `_roi_unreliable(ch)`+флаг `roi_unreliable`/`roi_caveat`+decompose_verdict через мост; hero-гард расширен (hero=Social verified); HTML таблица «ROI н/д*»+title, mROAS-комментарий заглушён.
**ОСТАЁТСЯ Волна 1:** (1) заглушка битых ROI — HTML CHART_DATA mROAS-граф (TRPs bar 15525), весь PPTX (таблица сл.8/findings/график), весь XLSX (лист ROI). Рекомендация: системно — data-level `mroas_display=None` для roi_unreliable, не латать каждое место. (2) model_reliability-плашка в отчёты (verdict/caveat_text/OVB через мост) — НЕ начато. (3) синхрон вердиктов таблицы (decompose vs optimize action_label; рекомендую honesty доминирует) — НЕ начато. **Волна 2** (XLSX waterfall-лист, PPTX 78→31/ESS/период, baseline-контекст) + **Волна 3** (метки режима/KPI, sales_share, глоссарий, версии, гигиена) — позже.
