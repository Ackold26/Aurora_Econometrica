...
**ОСТАЁТСЯ Волна 1:** (2) **model_reliability-плашка** — прокинуть из optimization.json через мост (`_map_pipeline` → data["diagnostics"]["model_reliability"]) + HTML(`sections.py`)/PPTX(`builder.py`) показывают verdict `uncertain`/reasons/OVB явной плашкой. Тон уже синхронизирован (Шаг 1); это добавляет ПОЛНОТУ (verdict/OVB/divergences). НЕ начато. (3) **синхрон вердиктов таблицы** — `derive_verdict`(=compute_channel_action) даёт «Uncertain», decompose action «Scale» → один канал два вердикта. Развилка (рекомендую: honesty доминирует при model_reliability.verdict!=reliable). НЕ начато. **Волна 2** (XLSX waterfall-лист молч.теряется — `report.rs:1075` `.as_array()` на объекте; PPTX «78→31»/ESS-фабрикация/период/«13→16 слайдов»; baseline 65% контекст во всех) + **Волна 3** (метки режима/KPI, sales_share, глоссарий XLSX 11→47, версии «v1.0.13», гигиена/англицизмы) — позже.
