...
- [x] **10.** ✅ ПУБЛИКАЦИЯ ВЫПОЛНЕНА (санкция Антона «1. Публикация», 2026-07-18, по регламенту
      ...
      · Тег `v2.4.0` → на сборочный SHA 1c1c6cf, запушен. Build Release job кода-репо:
        ✅ ОТКЛЮЧЁН (`752027c`, санкция Антона «исправь сразу», PR #5 → master): собирал exe
        без sidecar-бандла (артефакт битый, без смоук-гейта); канонический канал —
        aurora-releases по регламенту. Возврат — после добавления сборки sidecar в CI
        (условие в комментарии job). На тег v2.4.0 run даже не стартовал, авто-релиза нет.
