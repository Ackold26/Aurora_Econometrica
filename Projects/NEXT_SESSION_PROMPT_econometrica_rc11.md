...
1. Память: `project_econometrica_forecast_compare_phase_e_2026_06_13.md` (полный handoff forecast + аудит-секция), `feedback_anchor_session_to_one_real_artifact.md` (probe→фикстура→оракул→adversarial), `feedback_fix_at_shared_root_not_leaf_by_leaf.md` (N находок→руби по стволу), `feedback_echarts_tooltip_innerhtml_xss.md` (XSS-класс, кросс-продукт, INV-кандидат), `feedback_verify_existing_impl_before_building_planned_feature.md` (инвентаризация ДО оценки), `feedback_default_rhythm_probe_background_adversarial.md` (рабочий ритм), `feedback_autonomous_visual_testing_standard.md` (live-test грабли SvelteKit+ECharts §новая секция), `feedback_sidecar_rebuild_required.md`.
