...
7. **Артефакт с ЦЕПОЧКОЙ производных — перегенери ВСЕ уровни + прогони ВЕСЬ набор гейтов продукта локально ПЕРЕД wrap-up, не только линтер прямого изменения.** Глоссарий = ТРЁХуровневая цепочка: `источник (md + LEGACY_ONLY)` → `выходы (json/html/js)` → `PDF-справка (собирается из HTML)`. Совет #2 закрыл второй уровень (сверила json/html/js), но **PDF — третий, вторичный уровень — выпал**: я прогнала только `check_glossary_sync` (PASS) и решила, что чисто, а PDF нёс старый глоссарий (без OVB, со «доверительный интервал») → внешний аудит поймал как **High (F1)** → лишний цикл аудит→триаж→fix (`1ab729b`). Правило: первым ходом при правке генерируемого артефакта составь карту «что из чего генерируется», ВКЛЮЧАЯ **вторичные** уровни (PDF из HTML, bundle/installer из выходов, content-pack из данных); после правки источника перегенери ВСЕ уровни; и ПЕРЕД wrap-up прогони локально **весь** `.github/workflows/ci.yml` job `check` (все линтеры разом), не точечно — рассинхрон производных ловится ДО аудитора, дешевле. `check_help_pdf_consistency` теперь в CI+lefthook (закрыто F2), но привычка «прогнать весь набор гейтов перед сдачей» важнее одного линтера.
