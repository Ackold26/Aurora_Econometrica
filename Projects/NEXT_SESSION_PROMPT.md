...
### 🆕 Задачи Антона от 2026-07-17 (поставлены в очередь после wrap-up)
- **Запуск сразу на полный экран:** `maximized: true` в `src-tauri/tauri.conf.json` (окно);
  проверить наследование оверлеем `tauri.local.conf.json` + живьём dev и собранный exe.
- **Аудит no-window:** все спавны powershell/cmd/wmic/сайдкаров — без миганий консольных окон.
  Grep `Command::new` по src-tauri + `creation_flags(0x08000000)` CREATE_NO_WINDOW; частые
  источники — fingerprint (запросы серийников), updater (запуск установщика — окно может быть
  легитимно), claude.rs, sidecar-спавны. Классифицировать фикс/легитимно; верифицировать глазами на exe.
- **Справка: актуализация + загружаемый PDF** (крупная, возможно отдельная сессия): сверить
