...
> ✅ **P-2 РЕШЁН 2026-07-16 (коммит `85df196` на `feat/econ-kpi-units`, 696 pytest) — переквалифицирован.**
> Слайд «Прогноз на будущий период» ВСЕГДА был в PPTX (лог 2026-07-11 14:36: `forecast=yes`, слайд 6
> внутри секции «01 ГЛАВНОЕ») — «не находили» его потому, что он не имел следа в оглавлении, а таблица
> несла ЛОЖНЫЕ НУЛИ: `load_saved_forecast` читал вымышленные top-level ключи (total_kpi/total_spend_money/
> roas_money) вместо реальной схемы `totals.predicted_kpi` и подменял None→0.0; интервал был CI последнего
> периода (масштаб не совпадал с суммарным KPI). Исправлено: маппер (реальная схема + fallback легаси +
> None→«—» INV-50 + total_kpi_ci_low/high), PPTX+HTML (интервал СУММЫ за горизонт = числа GUI-карточки,
> SSOT), TOC-подстрока «в том числе «Прогноз на будущий период» — стр. NN», график сравнения (None-KPI
> исключается, CI-ключи выровнены). Доказано на реальной демо-фикстуре: слайд 6 = «★ Базовый план | — |
> 141 241 973 | 137 902 516 – 144 248 916 | —». Дифф-аудит чистым субагентом: 2 находки, обе закрыты.
> ⚠️ До клиентов доедет ТОЛЬКО с пересборкой sidecar (`build_sidecar.py`, V39) — выкат planning-линии разблокирован.
> 🆕 **Новый блокер в очереди: прод-баг 2.3.1 — sidecar 500 «module 'pyarrow' has no attribute '__version__'»**
> при загрузке файла с данными (скрин Антона 2026-07-16, тестовый комп). Гипотеза: PyInstaller несёт pyarrow
> без метаданных → pandas getattr(pyarrow,'__version__') падает. Смотреть `build_sidecar.py` hiddenimports/excludes.
>
> 🟢 ОБНОВЛЕНО 2026-07-11 (сессия аудит+P-1+живой прогон, 5 коммитов на `feat/econ-planning-mode`, НЕ запушено; предыдущее запушено до `6875b3c`):
> **P-1 ПОСТРОЕН И ДОКАЗАН ВЖИВУЮ** (мост :9223, собранный sidecar). Цепочка: аудит-фикс `7623366` (10 находок) → P-1 `1519828` (авто-baseline+манифест+кнопка btn-primary) → adversarial аудит-2 `25fa6bb` (7 находок в СВОИХ же правках: регресс двойного квалификатора A1, C2-divergence знаменателей, A-2 бюджет-₽ для физметрик, A-4 baseline до модели) → прокси-гейт `5b7bad8` (e2e на реальном движке) → живой прогон фиксы `36857cd` (график+даты). Гейты: pytest 558 · vitest 1031 · svelte 0 · cargo ok. **Sidecar ПЕРЕСОБРАН** (endpoint `econ_save_planning` боем 200 ok в собранном exe).
> **✅ P-1 живьём (демо-проект, AVT-мост):** baseline авто-строится (Выручка 141.3M + ДИ-веер [138.2–144.4]), карточка играет, **график история→прогноз рисуется** (вскрыт предсущ. баг `historicalSeries` — читал несуществующие `dData.dates`/`kpi_actuals` → всегда null → график НИКОГДА не рисовался; чинён: сумма `decomposition_series`), кнопка btn-primary синяя, `planning.json` автозаписан, A-2 (бюджет-₽ скрыт для физметрик).
> **🔴 ГЛАВНОЕ ОТКРЫТОЕ (→ след. сессия): P-2 — раздел «Прогноз на будущий период» НЕ попадает в PPTX.** Зонды исключили: `load_saved_forecast(демо)`=ok/1 scenario/6 preds; timing (planning.json+scenario ДО PPTX); `is_live`=True; `narrative_adapter:1119` форвардит `data['forecast']`; builder условие `if self.forecast:` (3975), `self.forecast` гейт `is_live AND status==ok AND scenarios` (builder:308) — ВСЕ формально истинны, раздел не рендерится. НЕ докопано. СЛЕД. ШАГ: прогнать генерацию PPTX через backend напрямую (HTTP `/report/*` на sidecar :7529 ИЛИ python narrative+builder) на демо, найти РЕАЛЬНЫЙ report-endpoint из GUI и где рвётся `self.forecast`. Полный зонд-лог → `TEST_FINDINGS_P1_live_2026_07_11.md`.
> **Прочее открытое:** F2 (лейбл cutoff «Дата отсечки» наложен на ось X — markLine label position) · #7 P-3 текст промиса · #3 демо-файлы FMCG+Pharma полный прогон · релиз v2.3.0 (installer aurora-fix + публикация по слову) · merge/push веток — решение Антона · #8 П2-1 drill (console.debug УБРАН в аудите-1/C1; диагностика теперь через `[EChartBase] setOption failed` логи).
> **Живой прогон — КАК (для повтора):** демо-проект «демо-медиаплан-с-хвостом» (модель+posterior). `media_plan.json` создавала ПРОГРАММНО: `validate_data`(auto не берёт кириллицу ТВ/ОЛВ → `no_media` error) → патчить `validation.json` роли media вручную → `confirm_media_plan(True)`. Мост `npm run tauri:dev` :9223, AVT (webview_execute_js/DOM-инспекция > скриншоты). SvelteKit-грабли: HMR сбрасывает сторы → переоткрыть проект (домой→Продолжить); навигация степпера `steps[5].click()`.
>
> — ниже прежний статус/приёмка 2026-07-10 (пункты 1,2 = P-1/P-2, ⚠️ P-1 закрыт, P-2 открыт по-новому выше) —
...
## 🗺 ДОРОЖНАЯ КАРТА КОММЕРЧЕСКОЙ ГОТОВНОСТИ → `COMMERCIAL_READINESS_ROADMAP.md`
Живой SSOT «до продажи»: 4 горизонта (гл. разрыв — пилот прогноз→факт, НЕ код) · трек **возврата Авроры Tier2** (отсутствует в сборке, на ветке `ai-insights-tier2`, 212 коммитов расхождения → ПОРТ не merge) + **RAG-слой** на облачной стороне · **🔒 решение Антона 2026-07-11: развиваем ПОКА только ОНЛАЙН-версию** (99% клиентов; офлайн по запросу на основе онлайн; инфраструктуру офлайн-сборки НЕ удалять — дремлющая база).
