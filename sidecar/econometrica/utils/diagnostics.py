...
# 🔴 Формулировка расширена 2026-08-08 по внешнему аудиту: расхождение бывает не
# только «другая модель». Пересчёт качества модели (tools/recompute_mqs.py) меняет
# диагностику, НЕ трогая саму модель: подпись остаётся прежней, а вердикт в
# результатах оптимизации — старым. Прежний текст про «другую модель» в этом
# случае был бы неправдой, поэтому говорим о разных расчётах.
PROVENANCE_MISMATCH_NOTE = (
    'Диагностика модели и результаты оптимизации получены в разных расчётах – '
...
# ── Единый источник фразы о применимости результата ──────────────────────────
#
# 🔴 Заведено 2026-08-09. До этого шкала отказа (utils/optimizer_honesty:
# `refused` при R-hat ≥ 1.05 или дивергенциях выше порога) и шкала показателя
# качества (MQS) жили порознь, а КАЖДЫЙ формат сам решал, что написать клиенту
# про применимость. Доказано зондом: R-hat 1.06, расхождений 0, R² 0.97,
# MAPE 3, отношение наблюдений к параметрам 12 → MQS 88 «Отличное» И
# одновременно `refused=True`. Клиент получал в одном документе «Модель не
# завершила расчёт корректно – рекомендации по переброске бюджета отключены»
# и «результаты модели надёжны для принятия решений».
#
# Приём тот же, что уже оправдал себя для оговорки о тонких данных
# (`format_thinness_caveat` выше): одна функция-источник, Rust зеркалит
# ДОСЛОВНО под тестом-сторожем. Правило потребления:
#   где печатается ступень MQS, там при отказе печатается эта фраза.
# Без отказа тексты форматов НЕ меняются — правка хирургическая.
#
# 🔴 Гейтим ДЕЙСТВИЕ, не ДАННЫЕ (записанный инвариант продукта): сам балл,
# ступень и метрики клиент видеть обязан. Запрещено только УТВЕРЖДЕНИЕ о
# надёжности и применение переброски. Поэтому фраза отказа прямо говорит, что
# цифры показаны как есть, — она ничего не прячет.
#
# Клиентский текст: короткое тире «–» (U+2013), без англицизмов.

#: Расчёт не сошёлся — показатель качества может быть каким угодно высоким.
#: 🔴 ЭТУ строку зеркалит Rust (src-tauri/src/commands/report.rs) — он не
#: импортирует Python и собирает Markdown и XLSX сам. Сторож шва:
#: tests/test_reliability_statement_mirror.py. Правя здесь, правь и там.
RELIABILITY_STATEMENT_REFUSED = (
    'Расчёт не сошёлся – цифры показаны как есть, но опираться на них при '
    'распределении бюджета рано; переобучите модель.'
)

#: Сошлось, данных достаточно, ступень хорошая или отличная.
RELIABILITY_STATEMENT_DEPENDABLE = 'Надёжный результат для принятия бюджетных решений.'

#: Ступень хорошая, но данных мало (honesty-аудит 2026-06-13, sub-finding b):
#: заявлять надёжность для бюджета нельзя — это противоречило бы оговорке о
#: тонких данных, которая печатается рядом.
RELIABILITY_STATEMENT_THIN = (
    'Формально модель качественна, но на ограниченных данных результаты '
    'ориентировочные.'
)

#: Средняя ступень.
RELIABILITY_STATEMENT_ACCEPTABLE = (
    'Приемлемо для ориентировочных решений, рекомендуем дополнительную валидацию.'
)

#: MQS-1 (2026-06-02): слабая ступень достигнута не низким качеством подгонки, а
#: ограничением по тонким данным при высокой объяснённой доле.
RELIABILITY_STATEMENT_OVERFIT = (
    'На коротких данных это вероятный признак переобучения, а не надёжности.'
)

#: Слабая ступень по существу — низкое качество подгонки.
RELIABILITY_STATEMENT_UNRELIABLE = (
    'Результаты ненадёжны - нужно больше данных или другая спецификация.'
)


def reliability_statement(mqs_tier: str, *, refused: bool = False,
                          thin: bool = False, high_fit: bool = False) -> str:
    """SSOT-фраза о применимости результата — согласованная с признаком отказа.

    Args:
        mqs_tier: ключ ступени MQS ('excellent'|'good'|'acceptable'|'weak'|'poor').
        refused: расчёт не сошёлся, рекомендации по переброске отключены
            (`utils.optimizer_honesty.model_did_not_converge` / поле `refused`
            в `model_reliability`, оно же вердикт 'unreliable').
        thin: применён предел по тонким данным (рядом печатается
            `format_thinness_caveat`).
        high_fit: высокая объяснённая доля (R² ≥ 0.7) — различает два пути к
            слабой ступени: реально плохую подгонку и переобучение на коротком ряде.

    🔴 Отказ старше ступени: показатель качества считается и при несошедшемся
    расчёте, поэтому «Отличное» и отказ встречаются вместе штатно — и именно
    тогда утверждать надёжность нельзя.
    """
    if refused:
        return RELIABILITY_STATEMENT_REFUSED
    if mqs_tier in ('excellent', 'good'):
        return RELIABILITY_STATEMENT_THIN if thin else RELIABILITY_STATEMENT_DEPENDABLE
    if mqs_tier == 'acceptable':
        return RELIABILITY_STATEMENT_ACCEPTABLE
    if thin and high_fit:
        return RELIABILITY_STATEMENT_OVERFIT
    return RELIABILITY_STATEMENT_UNRELIABLE
...
def generate_diagnostics_summary(r_squared: float, mape: float, rmse: float,
                                  ...
                                  total_draws: int | None = None) -> dict:
    ...
    total_draws (2026-08-09): всего черновиков MCMC (цепи × черновики). Нужен
    ТОЛЬКО для порога отказа по дивергенциям — max(20, 1% черновиков). Без него
    порог падает до абсолютного пола 20, и на длинных цепях текст вердикта
    объявил бы отказ там, где оптимизатор его не объявляет: ложная тревога и та
    же рассогласованность двух шкал, только в другую сторону. Передавать всюду,
    где число черновиков известно (modeler.py, tools/recompute_mqs.py).
    MQS-формула этим полем НЕ меняется — оно влияет только на текст вердикта.
    """
    ...
    # 🔴 Корень честности (2026-08-09). Прежде эта функция про сходимость не
    # спрашивала ВОВСЕ: фраза о надёжности выводилась из ступени MQS и тонкости
    # данных, хотя параметры сходимости ей передаются с самого начала. Отсюда
    # шло «Надёжный результат для принятия бюджетных решений» при несошедшемся
    # расчёте — а этот текст читает плашка на шаге ОБУЧЕНИЯ, то есть клиент
    # видел его ещё до оптимизации, где отказ только и объявлялся. Предикат
    # общий с `model_reliability_verdict` — разойтись они физически не могут.
    from utils.optimizer_honesty import model_did_not_converge
    refused = model_did_not_converge(r_hat_max, divergences, total_draws)

    # Зачин про объяснённую долю и фраза о применимости разведены намеренно:
    # зачин — про данные (его ветвление не трогаем), фраза — про то, что с
    # результатом можно делать, и она приходит из единого источника. Отказ
    # старше ступени, поэтому «только 20%» при несошедшемся расчёте остаётся
    # честным зачином, а вывод делает уже фраза.
    statement = reliability_statement(
        mqs['tier'], refused=refused, thin=bool(thin_note), high_fit=r_squared >= 0.7,
    )
    if mqs['tier'] in ('excellent', 'good', 'acceptable'):
        lead = f"Модель объясняет {r2_pct}% вариации продаж (R²)."
    elif thin_note and r_squared >= 0.7:
        ...
        lead = f"Модель объясняет {r2_pct}% вариации продаж (R²) — высокий fit."
    else:
        lead = f"Модель объясняет только {r2_pct}% вариации продаж (R²)."
    # honesty-аудит 2026-06-13 (sub-finding b) и далее: «надёжно для бюджетных
    # решений» при тонких данных или несошедшемся расчёте не заявляется — это
    # теперь решает `reliability_statement`, а не ветвление здесь.
    verdict = f"{lead} {statement}{thin_note}"
