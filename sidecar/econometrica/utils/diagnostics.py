...
def format_thinness_caveat(ratio: float | None, thinness_cap: int | None,
    ...
    if thinness_cap is None or ratio is None:
        return ""
    # Тон по McElreath (regularizing priors, синхрон с optimizer_honesty.py):
    # тонкие данные ≠ поломка/«артефакт переобучения». Информативные priors
    # регуляризуют, стягивая оценки к разумному диапазону, а широкие интервалы
    # честно отражают неопределённость. Формулируем сдержанность модели.
    if ratio < 2:
        body = (f"⚠ Данных мало (Ratio {ratio:.1f}:1) — их совсем немного: модель "
                f"сильно опирается на априорные отраслевые знания, оценки "
                f"ориентировочные. Доверяйте доверительным интервалам, а не "
                f"точечным цифрам.")
    else:
        body = (f"⚠ Данных мало (Ratio {ratio:.1f}:1 < 4:1): модель намеренно "
                f"сдержана — опирается на априорные отраслевые знания, поэтому "
                f"доверительные интервалы широкие. Это честная неопределённость, "
                f"а не ошибка; с ростом данных интервалы сузятся.")
...
def generate_diagnostics_summary(r_squared: float, mape: float, rmse: float,
    ...
    if mqs['tier'] in ('excellent', 'good') and not thin_note:
        ...
        # fit ИЛИ высокий R², зажатый data-thinness cap (короткий ряд). «объясняет
        # только 98%» вводит в заблуждение: 98% — высокий fit, но на коротких данных
        # точечная надёжность ограничена (тон McElreath, синхрон с optimizer_honesty:
        # priors стягивают оценки, интервалы честно широкие). INV-50: честно про корень.
        verdict = (
            f"Модель объясняет {r2_pct}% вариации продаж (R²) — высокий fit. "
            f"На коротких данных точечная надёжность ограничена: модель опирается на "
            f"априорные знания, поэтому опирайтесь на доверительные интервалы.{thin_note}"
        )
    else:
