...
def environment_snapshot(
    *,
    ...
    Args:
        ...
        chain_method: способ раскладки цепей, ЗАПРОШЕННЫЙ движком. Именно
            запрошенный, а не применённый — см. ``chain_method_delivered``.
    ...
    return {
        ...
        # 🔴 Проверено зондом 2026-08-02 на PyMC 5.28.4: аргумент
        # `chain_method`, переданный напрямую в `pm.sample`, до NumPyro НЕ
        # доезжает — `_sample_external_nuts` пробрасывает во внешний сэмплер
        # только `nuts_sampler_kwargs`, а прочее молча проглатывает. Значит
        # применяется дефолт PyMC (`parallel`), который сам NumPyro понижает
        # до `sequential`, когда устройств меньше, чем цепей.
        # Пока это так, паспорт не имеет права называть запрошенную раскладку
        # применённой: он утверждал бы то же, чего мы не делали.
...
def mark_chain_layout(
    ...
    *,
    ...
    """Записать в снимок ЗАПРОШЕННУЮ раскладку цепей и число устройств.
    ...
    Запрошенная раскладка сегодня до сэмплера не доезжает (см. поле
    ``chain_method_delivered``), но в паспорте нужна: она объясняет,
    почему движок ожидал именно такую раскладку, и станет применённой,
    как только передача будет починена.
    """
    if snapshot is None:
        return
