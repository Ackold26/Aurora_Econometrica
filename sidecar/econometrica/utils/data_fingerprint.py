...
def describe_frame_algorithm() -> dict[str, Any]:
    """Правила канонизации таблицы – в виде, пригодном для чужой реализации.

    Зачем отдельной функцией, а не текстом в модуле выгрузки: отпечаток без
    описания правил непроверяем (посторонний не может пересчитать хеш и
    сверить его), а описание, живущее в другом файле, расходится с
    реализацией на первой же правке. Поэтому описание порождается ЗДЕСЬ,
    рядом с самим алгоритмом, и подставляет фактические константы модуля –
    смена ``NUM_SIGNIFICANT_DIGITS`` или разделителя меняет и описание.

    Проверка описания на полноту делается так: по этому тексту пишется
    независимая реализация (несколько строк на pandas и hashlib) и её хеш
    сверяется с выдачей ``compute_frame_fingerprint`` – см.
    ``tests/test_data_fingerprint_selfdesc.py``.

    Returns:
        Словарь с правилами: чему соответствует ячейка, как считается хеш
        столбца, как – хеш таблицы, к чему алгоритм чувствителен.
    """
    na_hex = ' '.join(f'0x{b:02X}' for b in _NA_TOKEN.encode('utf-8'))
    return {
        'algo': FRAME_ALGO,
        'hash_function': 'SHA-256',
        'digest_format': 'шестнадцатеричный текст, 64 знака, нижний регистр',
        'source': (
            'utils/data_fingerprint.py – compute_frame_fingerprint, _column_digest, _canon_scalar'
        ),
        'what_is_hashed': (
            'Таблица исходных данных так, как её прочитала программа: все столбцы файла '
            'в исходном порядке и все строки в исходном порядке. Отбор столбцов под модель '
            'происходит позже и на отпечаток не влияет.'
        ),
        'constants': {
            'algo_label': FRAME_ALGO,
            'significant_digits': NUM_SIGNIFICANT_DIGITS,
            'cell_separator_hex': f'0x{_CELL_SEP[0]:02X}',
            'field_separator_hex': f'0x{_FIELD_SEP[0]:02X}',
            'na_token_bytes_hex': na_hex,
        },
        'cell_canonical_form': [
            'Пропуск (None, NaN, NaT, pd.NA), а также текст «nan», «NaN», «NaT», «<NA>» – '
            f'единый токен из байтов {na_hex}.',
            'Логическое значение – текст «TRUE» или «FALSE» (не 1 и не 0: в таблице это '
            'отдельный тип).',
            'Целое число – десятичная запись без знака плюс и без разделителей разрядов '
            '(результат str(int(значение))).',
            'Дробное число – ноль записывается как «0» (без знака минус), плюс-минус '
            'бесконечность – как «inf» и «-inf», прочее – формат Python '
            f'«%.{NUM_SIGNIFICANT_DIGITS}g» (то есть {NUM_SIGNIFICANT_DIGITS} значащих цифр, '
            'экспоненциальная форма при необходимости).',
            'Дата и время – ISO-8601, результат метода isoformat() '
            '(например «2023-01-01T00:00:00»).',
            'Всё остальное – текстовое представление значения как есть.',
            'Скаляр numpy сначала разворачивается в родной тип Python (np.int64 → int, '
            'np.bool_ → bool), после чего применяются правила выше.',
        ],
        'column_digest': [
            'Завести SHA-256.',
            f'Для каждой строки в порядке таблицы подать: канонический текст ячейки в UTF-8, '
            f'затем разделитель ячеек 0x{_CELL_SEP[0]:02X}.',
            'Хеш столбца – шестнадцатеричный дайджест (64 знака, нижний регистр).',
        ],
        'frame_digest': [
            'Завести SHA-256.',
            f'Подать метку алгоритма «{FRAME_ALGO}» в UTF-8, затем разделитель полей '
            f'0x{_FIELD_SEP[0]:02X}.',
            f'Подать текст «rows=<число строк>;cols=<число столбцов>» в UTF-8, затем '
            f'0x{_FIELD_SEP[0]:02X}.',
            'Для каждого столбца в порядке таблицы (нумерация с нуля) подать: номер позиции '
            'десятичным текстом, разделитель полей, имя столбца в UTF-8 буква в букву '
            '(включая переводы строк внутри имени), разделитель полей, хеш столбца из '
            'предыдущего пункта как текст ASCII, разделитель полей.',
            'Отпечаток содержимого – шестнадцатеричный дайджест итогового SHA-256.',
        ],
        'sensitive_to': [
            'значению любой ячейки',
            'имени столбца',
            'порядку столбцов',
            'порядку строк',
            'числу строк и столбцов',
            'метке алгоритма (смена правил обязана менять метку)',
        ],
        'insensitive_to': [
            'представлению числа (целое / дробное, версия библиотеки чтения)',
            'способу записи пропуска',
            'пересохранению файла',
            'имени и пути файла',
        ],
        'note': (
            'Описание порождается тем же модулем, что считает отпечаток, и подставляет его '
            'фактические константы – расхождение описания с расчётом исключено по построению.'
        ),
    }
