...
def decompose(
    ...
    if model_version == '1.0-ols':
        ...
        model_warning = (
            ...
            f'Правдоподобные диапазоны - frequentist на β-коэффициенты + предиктивные '
            f'интервалы на y. Правдоподобные диапазоны на ROI/mROAS недоступны (нужен n≥30 для '
        ...
        model_warning = (
            ...
            'Правдоподобные диапазоны не учитывают неопределённость carryover. '
            'Переобучите модель для получения честного правдоподобного диапазона на adstock (Phase 1.1, v1.2).'
        )
    elif model_version == '1.1':
        model_warning = (
            'Эта модель обучена до Phase 1.9 - посterior samples отсутствуют. '
            'Правдоподобные диапазоны недоступны. Переобучите модель для поддержки правдоподобных диапазонов.'
        )
