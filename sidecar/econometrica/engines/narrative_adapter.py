...
def derive_action_headline(
    ...
    # Теперь стороны — из общего правила, одного на весь продукт.
    # 🔴 s48 (14.09.2026, Projects/THRESHOLDS_s48.md): здесь стоял свой порог
    # значимости — 1 млн ₽, обоснованный тем, что «единственный потребитель этой
    # функции — колода, а её слайды считают переброску значимой именно с этой
    # величины». Верно для этой функции, но не для соседних мест ТОЙ ЖЕ колоды
    # (`aurora_pptx/builder.py`, слайд «Главное»), которые звали
