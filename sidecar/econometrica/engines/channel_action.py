...
def compute_channel_action(
    ...
    *,
    ...
    # 10.09.2026: `<=`, не `<` - оптимизатор при нижней границе (напр. 50% от baseline)
    # упирает канал РОВНО в ratio=0.5, и строгое сравнение пропускало этот случай мимо Cut
    # (проверено: 0.499999 → Cut, 0.5 → мимо, 0.500001 → мимо; асимметрия без причины).
    ...
    # 10.09.2026: было `eff_mroas > 0 and eff_mroas < 0.8` - отрицательная предельная
    # отдача (канал ВЫЧИТАЕТ вклад, не просто не окупается) не попадала ни в эту ветку,
    # ни в одну из следующих (все требуют eff_mroas > 0) и проваливалась в fallback
    # Watch/«Под наблюдением» - самый мягкий вердикт для самого убыточного канала.
    # Условие расширено до eff_mroas < 0.8 (без нижней границы), reasoning ниже честно
    # называет отрицательный случай отдельно.
    if eff_mroas < 0.8:
        # 09.09.2026. Эта ветка стоит выше веток, смотрящих на решение оптимизатора, и до
        # `ratio` управление не доходило. Пока решатель останавливался рано, это не было
        # видно; после его починки оптимизатор стал ДОБАВЛЯТЬ бюджет убыточному каналу —
        # и рядом со знаком «плюс» вставал вердикт «Сократить» с пояснением «каждый рубль
        # приносит убыток». Читается как противоречие продукта самому себе.
        #
        # Причина роста бюджета настоящая, а не сбой: задача решается при ЗАФИКСИРОВАННОЙ
        # общей сумме (ограничение-равенство Σ бюджетов = целевой бюджет). Деньги нельзя
        # не потратить, поэтому когда остальные каналы упёрлись в насыщение или в границы,
        # оптимум доливает наименее убыточному из оставшихся. Вердикт «Сократить» при этом
        # по существу верен — неверно было бы советовать переклад внутри той же суммы.
        #
        # Поэтому ключ, тон и приоритет не меняются (канал действительно убыточен), меняется
        # только пояснение: оно называет причину, по которой бюджет здесь не сокращён.
        ...
        if ratio > 1.0:
            return ChannelAction(
                ...
                tone=ACTION_TONE['Cut'],
                reasoning=(
                    f'{_below_breakeven_str}. '
                    f'Бюджет здесь не сокращён потому, что общая сумма зафиксирована, '
                    f'а остальные каналы уже насыщены: выигрыш даёт сокращение общего '
                    f'бюджета, а не переклад внутри него'
                ),
                priority=ACTION_PRIORITY['Cut'],
                confidence=confidence,
            )
