...
def _каналы(модель, конфиг, нормировка, выборки, нет) -> Dict[str, Any]:
    ...
    for канал, параметры in параметры_каналов.items():
        ...
        примечания['media_mean'] = (
            'среднее ряда ПОСЛЕ переноса с фиксированным коэффициентом 0,5 – точка нормировки '
            'насыщения, а не средние траты канала и не апостериорный перенос'
        )
...
def _нормировка(нормировка, нет) -> Dict[str, Any]:
    ...
    return {
        ...
        'note': (
            ...
            'а не средние трат канала: они служат точкой нормировки насыщения. Сравнивать их '
            'с channels[*].adstock_mean_posterior (перенос с апостериорным коэффициентом) нельзя – '
            'у этих чисел разные основания, и меньшее значение переноса тут не противоречие.'
        ),
    }
