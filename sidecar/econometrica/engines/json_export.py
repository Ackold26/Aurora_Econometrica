...
def _перенос_канала(имя, параметры, типы_переноса, протокол_переноса,
    ...
    if записанный in _ФАКТИЧЕСКИЕ_ТИПЫ_ПЕРЕНОСА:
        ...
        if записанный == 'auto':
            примечания['adstock_type'] = (
                ...
                'а не что применилось. Восстановить фактический тип задним числом нельзя. '
                'Расчёт при обучении шёл по геометрическому откату с фиксированным значением '
                'из кода продукта (см. specification.adstock_defaults_in_code).'
            )
            нет(
    ...
    if isinstance(вейбулл, dict) and вейбулл:
        ...
        примечания['weibull_params'] = (
            ...
            'значения по умолчанию – в specification.adstock_defaults_in_code'
        )
        нет(
            f'channels["{имя}"].weibull_params',
            'параметры вейбулловского переноса (shape / scale / max_lag) в модели не записаны',
            'значения по умолчанию из кода продукта: specification.adstock_defaults_in_code',
        )
