...
🔴 Правило модуля: НИКАКОЙ ПОДСТАНОВКИ УМОЛЧАНИЙ.
    ...
    Отдельно и не менее важно: умолчания подставляет не только этот модуль, но и
    ЗАГРУЗЧИК моделей – `engines/persistence.load_model_with_compat` кладёт
    `kpi_type='sales'`, `kpi_likelihood='normal'`, `kpi_kind`, `model_version='1.0'`,
    `per_channel_input`. Что именно подставлено, загрузчик оставляет следом
    (`LOADER_DEFAULTS_KEY`), а документ этот след читает: подставленное поле
    выходит пустым с происхождением `loader_default` и записью в `absent_fields`.

История схемы:
    3.2 (2026-08-16) – закрыт Critical C-1 внешнего аудита: умолчание загрузчика
        уезжало в документ под видом записи модели. У модели без верхнеуровневых
        полей KPI документ печатал `type: "sales"` с пометкой «прочитано из файла
        модели как есть», а `absent_fields` про KPI молчал: модель, обученная на
        знании марки, заверялась как модель продаж. Добавлено происхождение
        `loader_default`, поля KPI, версия схемы модели и единицы каналов
        размечаются по следу загрузчика, а не по наличию значения.
...
ПОДСТАВЛЕНО = 'loader_default'  # в файле модели поля нет – значение подставил загрузчик при чтении
...
_РАСШИФРОВКА_ИСТОЧНИКОВ = {
    ...
    ПОДСТАВЛЕНО: 'в файле модели такого поля нет, а загрузчик продукта при чтении подставил '
                 'своё умолчание. В документ оно не выносится (значение пустое): это допущение '
                 'программы, а не запись обучения. Что именно подставлено – в absent_fields',
...
def _подставленные_загрузчиком(модель: Dict[str, Any]) -> set:
    """Имена полей, которых в файле модели не было – их подставил загрузчик.

    След оставляет `engines/persistence.load_model_with_compat`, имя ключа берём
    оттуда же: две копии одной строки рано или поздно разъезжаются. Следа нет
    (модель собрана в памяти или прочитана мимо загрузчика) – множество пустое,
    и разметка источников работает как прежде, по наличию значения.
    """
    try:
        from engines.persistence import LOADER_DEFAULTS_KEY as ключ
    except ImportError:  # pragma: no cover – модуль лежит рядом
        ключ = '_loader_injected_defaults'
    след = модель.get(ключ)
    if not isinstance(след, (list, tuple, set)):
        return set()
    return {имя for имя in след if isinstance(имя, str)}
...
def export_model_params_json(
    ...
    payload['schema'] = {
        ...
        'honesty_rule': (
            ...
            'Это касается и умолчаний, которые подставляет сама программа при чтении файла '
            'модели: такое поле выходит пустым с происхождением loader_default, а подставленное '
            'значение названо в absent_fields как допущение программы.'
        ),
    ...
    # 🔴 Часть полей загрузчик продукта подставляет сам при чтении файла
    # (`engines/persistence.py`, LOADER_DEFAULTS_KEY): нет в файле kpi_type –
    # будет 'sales', нет kpi_likelihood – 'normal', нет model_version – '1.0'.
    # Выдавать подставленное за запись обучения нельзя: модель, обученная на
    # знании марки, уходила третьей стороне моделью продаж (Critical C-1,
    # внешний аудит 2026-08-16). Различаем по следу загрузчика, а не по догадке
    # о версии схемы: догадка в заверяемом документе – тот же домысел.
    подставлено_загрузчиком = _подставленные_загрузчиком(модель)

    def строковое_поле(имя: str, поле_документа: str, где: str) -> Tuple[Optional[str], str]:
        """Строковое поле модели и честное происхождение его значения.

        Подставлено загрузчиком – в документ уходит пустота, а не умолчание, и
        подстановка называется поимённо в absent_fields.
        """
        значение = _как_строку(модель.get(имя))
        if имя in подставлено_загрузчиком:
            нет(
                поле_документа,
                f'в файле модели поля {имя} нет; при чтении загрузчик продукта подставил '
                f'«{значение}» – это умолчание программы, а не запись обучения',
                где,
            )
            return None, ПОДСТАВЛЕНО
        if значение is None:
            нет(поле_документа, f'поле {имя} в файле модели не записано', где)
            return None, НЕ_ЗАПИСАНО
        return значение, ЗАПИСАНО

    # ─── Модель ─────────────────────────────────────────────────────
    версия_модели, происхождение_версии = строковое_поле(
        'model_version', 'model.version',
        'версия схемы модели: у файлов ранних версий её нет, загрузчик продукта считает такие '
        'модели версией 1.0',
    )
    иерархическая = модель.get('use_hierarchical')
    payload['model'] = {
        'version': версия_модели,
        'version_origin': происхождение_версии,
    ...
    # ─── KPI ────────────────────────────────────────────────────────
    kpi_type, происхождение_типа = строковое_поле(
        'kpi_type', 'kpi.type',
        'фактический тип KPI знает тот, кто обучал модель; косвенно о нём говорит столбец '
        'исходных данных, названный в kpi.column',
    )
    kpi_kind, происхождение_рода = строковое_поле(
        'kpi_kind', 'kpi.kind',
        'род величины выводится из типа KPI (utils/kpi_registry.py); то же значение стоит '
        'в kpi.unit_kind',
    )
    kpi_likelihood, происхождение_правдоподобия = строковое_поле(
        'kpi_likelihood', 'kpi.likelihood',
        'вид правдоподобия при обучении знает тот, кто обучал модель',
    )
    ...
    # Подпись единицы читается по следу, а не голым `.get`: загрузчик кладёт сюда
    # пустую строку, и пока она пуста, разницы не видно – но правило блока не
    # «пустое умолчание безвредно», а «умолчание не выдаётся за запись». Сторож
    # `tests/test_loader_defaults_ratchet.py` поймал это подменной меткой.
    подпись_единицы, происхождение_подписи = строковое_поле(
        'value_per_count_unit_label', 'kpi.unit_label',
        'подпись единицы счётного KPI задаётся в шаге проверки данных; у денежных '
        'и долевых KPI её не бывает',
    )
    payload['kpi'] = {
        'type': kpi_type,
        'unit_kind': kpi_kind,
        'unit_kind_origin': происхождение_рода,
        'unit_kind_note': (
        ...
        'unit_label_origin': происхождение_подписи,
        'type_origin': происхождение_типа,
        'kind': kpi_kind,
        'kind_origin': происхождение_рода,
        'likelihood': kpi_likelihood,
        'likelihood_origin': происхождение_правдоподобия,
        'column': kpi_column,
...
def _каналы(модель, конфиг, нормировка, выборки, нет) -> Dict[str, Any]:
    ...
    единицы_подставлены = 'per_channel_input' in _подставленные_загрузчиком(модель)
    ...
    for канал, параметры in параметры_каналов.items():
        ...
        # Единицы каналов загрузчик тоже подставляет – всем каналам 'monetary',
        # если поля в файле нет (та же подстановка, что у KPI: Critical C-1).
        род_единицы = None if единицы_подставлены else _как_строку(единицы_каналов.get(имя))
        запись['unit_kind'] = род_единицы
        источники['unit_kind'] = (
            ПОДСТАВЛЕНО if единицы_подставлены
            else (ЗАПИСАНО if род_единицы else НЕ_ЗАПИСАНО)
        )
        ...
        if род_единицы is None:
            нет(
                ...
                (
                    'в файле модели поля per_channel_input нет; при чтении загрузчик продукта '
                    'объявил все каналы денежными – это умолчание программы, а не запись обучения'
                ) if единицы_подставлены else
...
def _приоры(модель, конфиг, нормировка, нет) -> Dict[str, Any]:
    ...
    # Как и у KPI, читаем по следу загрузчика, а не по наличию значения: сюда
    # он кладёт пустую запись, и сегодня она безобидна – но безобидна значением,
    # а не устройством. Сторож `tests/test_loader_defaults_ratchet.py` вскрыл
    # это подменной меткой: непустая заглушка ушла бы в документ как запись обучения.
    знаковые = модель.get('signed_factor_priors_used')
    if 'signed_factor_priors_used' in _подставленные_загрузчиком(модель):
        блок['signed_factor_priors'] = None
        блок['signed_factor_priors_origin'] = ПОДСТАВЛЕНО
        нет(
            'priors.signed_factor_priors',
            'в файле модели поля signed_factor_priors_used нет; при чтении загрузчик '
            'продукта подставил пустую запись – это умолчание программы, а не запись обучения',
            'приорные средние по каждому контролю – priors.control_prior_means',
        )
    elif isinstance(знаковые, dict) and знаковые:
