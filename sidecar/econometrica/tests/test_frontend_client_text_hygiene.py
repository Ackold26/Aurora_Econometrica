...
_LEGITIMISED_ADSTOCK_FILES = {
    ...
    "lib/components/OnboardingOverlay.svelte": (
        "описание слайда «Модели и методы» в онбординге: перечень применяемых "
        "методов рядом с их принятыми названиями. 🔴 Пограничный случай — "
        "онбординг видят ВСЕ, не только продвинутые пользователи, и вокруг "
        "стоит целый кластер англоязычных терминов. Узаконено, чтобы не "
        "держать проверку красной, но помечено владельцу отдельно: решение "
        "об уровне русификации онбординга целиком ещё не принималось"
    ),
}
