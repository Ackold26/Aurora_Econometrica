...
def test_rs_report_no_spaced_hyphen_as_dash():
    ...
    """
    v = _collect_violations()
    # Реестр узаконенных литералов действует и здесь (внешний аудит 2026-07-28, две находки):
    # правило не различает тире от арифметического минуса («прирост 10 - 15 %») и краснеет на
    # маркере списка, если перед ним стоит Rust-экран переноса внутри литерала («…\n - пункт»:
    # символ «n» непробельный, поэтому lookbehind срабатывает). Оба случая — законный текст, а
    # не дефект. Механизма исключений у правила не было вовсе; теперь тот же реестр по ТЕКСТУ
    # литерала, что у baseline/adstock, покрывает и П8-4 — новая законная форма заносится туда
    # с обоснованием, а не заставляет ослаблять само правило.
    unexpected = [
        (ln, lit) for ln, lit in v["spaced_hyphen"]
        if lit.strip() not in _LEGITIMISED_LITERALS
    ]
