...
# ─── Сторож: формат суммы переброски живёт в одном месте ─────────────────────
#
# 🔴 Аудит s48b (High-1): порог и формат суммы переброски объявлены рядом, в
# ОДНОМ узле (`utils.optimizer_honesty`), но сторожем выше стерегли только
# порог. Формат жил без охраны — и в `aurora_pptx/builder.py` завёлся второй,
# локальный `_fmt_mln(v)`, печатавший «1,0 млн» (с нулевой десятой), пока
# сосед той же колоды печатал «1 млн ₽» через общую `format_realloc_mln`.
# Ведущая сессия сняла `_fmt_mln` и перевела три печати на общую функцию —
# этот тест стережёт, чтобы дубль не завёлся снова, под любым именем.
#
# Рассматривались два подхода:
#
#   (A) искать в исходниках форматирование с плавающей точкой (`:.1f`/`:.0f`)
#       рядом со словом «млн». Отклонён: слово «млн» стоит на строке ВЫЗОВА
#       (`f"...{_fmt_mln(...)} млн"`), а сам формат-спецификатор — внутри ТЕЛА
#       вспомогательной функции, часто на другой строке в другом месте файла;
#       регэксп с окном в несколько строк эту связь не видит. Хуже: в этих же
#       файлах формат `:.0f} млн` легитимно печатает СОВСЕМ ДРУГУЮ сумму —
#       общий бюджет (`budget:.0f} млн ₽`, `tb:.0f} млн руб`, `_tb_mln:.0f}
#       млн руб`) — тот же приём дал бы ложные срабатывания на каждый такой
#       вызов. Этот подход буквально НЕ поймал бы реальный дефект s48b и ловил
#       бы чужие места мимо цели.
#   (B) [ВЫБРАН] требовать, чтобы носитель суммы переброски — идентификатор
#       `realloc` или подстрочный доступ `...['amount_mln']`/`["amount_mln"]`
#       — на месте печати был обёрнут ИМЕННО вызовом `format_realloc_mln(...)`,
#       а не какой-либо другой функцией и не голым. Это класс дефекта, а не
#       имя `_fmt_mln`: сторож ловит обёртку любым чужим именем (`_money`,
#       `_fmt`, будущий синоним) и голую интерполяцию без обёртки вовсе.
#       Проверено этим же приёмом на реальном дефекте: `_fmt_mln(_f3_subjects
#       ['amount_mln'])` — носитель обёрнут НЕ в `format_realloc_mln`, сторож
#       красный.
#
# Слабое место (B), названное прямо: сторож опознаёт носитель СУММЫ по имени
# (`realloc` / ключ `amount_mln`). Новое место, где сумма переброски попадёт
# в переменную с другим именем (например `rebalance_amount`), появится вне
# поля зрения сторожа — тот же класс слепоты, что уже принят выше у сторожа
# порога (`_INLINE_THRESHOLD_RE` тоже ловит только `reallocation_mln`/
# `realloc`). Второе слабое место: построчный разбор (как и у сторожа порога
# выше) не видит вызов, у которого аргумент переносится на следующую строку.
_FORMAT_GUARDED_FILES = (
    os.path.join(os.path.dirname(_HERE), "aurora_pptx", "builder.py"),
    os.path.join(os.path.dirname(_HERE), "engines", "narrative_adapter.py"),
    os.path.join(os.path.dirname(_HERE), "aurora_html", "sections.py"),
)

# Носитель суммы переброски: голый идентификатор `realloc` (не часть слова
# `reallocation_subjects`/`reallocation_mln` — `\b` не входит внутрь них, там
# нет границы слова между «c» и «a»/«t»), либо подстрочный доступ к ключу
# `amount_mln` через одинарные или двойные кавычки. Отрицательный просмотр
# вперёд исключает строки-присваивания вида `realloc = facts.get(...)` —
# это ввод сырого значения, а не место печати, там охранять нечего.
_AMOUNT_CARRIER_RE = re.compile(
    r"\brealloc\b(?!\s*=(?!=))"
    r"|\w+\[\s*['\"]amount_mln['\"]\s*\](?!\s*=(?!=))"
)
# Непосредственно перед носителем (с точностью до пробелов) обязан стоять
# именно этот вызов — обёртка чужим именем или голая интерполяция не пройдут.
_PROPER_WRAP_RE = re.compile(r"format_realloc_mln\(\s*$")


def test_no_inline_reallocation_format_literal_outside_shared_source():
    """Сумма переброски печатается только через `format_realloc_mln` — здесь
    её обёртывать чем-то другим или печатать голой нельзя.

    Формальный вариант (позвать `format_realloc_mln` дважды и сравнить с самой
    собой) был бы зелёным по построению; этот тест читает исходники
    продакшн-кода и ищет носитель суммы, обёрнутый не в общую функцию.
    """
    offenders = []
    for path in _FORMAT_GUARDED_FILES:
        with open(path, encoding="utf-8") as fh:
            for n, line in enumerate(fh, 1):
                stripped = line.lstrip()
                if stripped.startswith("#"):
                    continue
                for m in _AMOUNT_CARRIER_RE.finditer(line):
                    prefix = line[:m.start()]
                    if _PROPER_WRAP_RE.search(prefix):
                        continue
                    offenders.append(
                        f"{os.path.basename(path)}:{n}: {line.strip()}"
                    )
    assert not offenders, (
        "Сумма переброски печатается в обход общего форматтера "
        "utils.optimizer_honesty.format_realloc_mln (обёрнута другой функцией "
        "или напечатана без обёртки):\n" + "\n".join(offenders)
    )
