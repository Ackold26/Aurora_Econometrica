...
def test_rust_mirrors_python_meaning(idx: int, name: str) -> None:
    ...
    # 🔴 Симметрия: односторонняя доля |py∩rs|/|py| пропускала ДОПОЛНЕНИЕ Rust любым
    # текстом (найдено аудитом: дописать «Результаты использовать нельзя» — сторож
    # зелёный, клиент получает в XLSX алармизм, которого нет в HTML/PPTX).
    extra = rs_words - py_words
    assert len(extra) <= max(3, len(py_words) // 4), (
        f"{name} оговорка в Rust несёт лишние смысловые слова, которых нет в "
        f"Python-эталоне: {sorted(extra)[:8]}. Зеркало обязано быть дословным — "
        f"дополнять одну сторону нельзя, клиент получит разный текст в разных форматах."
    )
    assert overlap >= 0.7, (
...
def test_rust_does_not_carry_abandoned_alarmism() -> None:
    ...
    # 🔴 Ищем КОРНИ, а не точные строки: аудит показал, что «результаты
    # ненадёжные», «результаты не надёжны», «риск переобучения высокий» проходили
    # насквозь мимо списка точных фраз.
    banned_patterns = [
        (r"результат\w*\s+(?:не\s*)?надёжн", "утверждение о ненадёжности результатов"),
        (r"артефакт\w*\s+переобучени", "артефакт переобучения"),
        (r"риск\w*\s+переобучени|переобучени\w*\s*[–—-]?\s*высок", "алармизм про риск переобучения"),
    ]
    for pattern, human in banned_patterns:
        found = re.search(pattern, code, re.I)
        assert not found, (
            f"в клиентский текст report.rs вернулся отброшенный оборот ({human}): "
            f"«{code[max(0, found.start()-30):found.end()+30]}». Единый источник — "
            f"utils/diagnostics.py::format_thinness_caveat, он описывает механизм."
        )
