...
def test_function_is_wired_into_builders(
    ...
    for builder in builders:
        ...
        # 🔴 Наличия вызова НЕДОСТАТОЧНО — результат может быть отброшен.
        # Найдено внешним аудитом и доказано мутацией: замена живых вызовов на
        # `{ let _dead = clean_label(x); x }` сохраняет вызов, выбрасывает
        # результат и оставляет сторож зелёным — то есть первая редакция этого
        # сторожа проверяла ровно то, что легко обойти, не заметив.
        discarded = re.findall(
            rf"let\s+_\w*\s*(?::[^=]+)?=\s*{re.escape(func)}\s*\(", body
        )
        assert not discarded, (
            f"в {builder} результат {func} присваивается отбрасываемой переменной "
            f"(`let _…`): вызов формально есть, а дальше идёт исходное значение. "
            f"Следствие для клиента: {consequence}."
        )
