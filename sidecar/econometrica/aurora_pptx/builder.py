...
def строки_сертификата(cert) -> list[tuple[str, str]]:
    ...
    # H-4 (внешний аудит 2026-08-16): критерий совпадения считался движком, но
    # не печатался ни в отчёте, ни здесь. `подробно=False` – краткая форма
    # слайда (одна строка про ветвь, применимую к ЭТОМУ расчёту), НЕ подробная
    # форма отчёта: список диагностики слайда «Методология» и так ужимается
    # под восемь строк (см. вызывающий код – шаг автосжимается по числу строк).
...
class AuroraPPTXBuilder:
    ...
    def _build_at_a_glance_findings(self):
        ...
        if mqs is None:
            ...
            # расходилась с каноном 85/70/55/40: MQS 70 давал «Хорошее» в
            # отчёте и «приемлемо» на слайде, MQS 55-59 — «Приемлемое» против
            # «требует доработки». Это ровно тот инцидент, ради которого SSOT
            # и заводился (см. комментарий у _MQS_TIERS).
            from utils.diagnostics import mqs_tier_info
            _tier = mqs_tier_info(mqs)
            f5 = f"Качество модели: MQS {mqs:.0f}/100 – {_tier['tier_label'].lower()}"
            s5 = {
                'excellent': "Можно опираться на рекомендации в планировании",
                'good': "Можно опираться на рекомендации в планировании",
                'acceptable': "Рекомендации валидны с учётом диагностических метрик",
        ...
        for i, (name, pg) in enumerate(zip(self.section_names, self.toc_page_refs), start=1):
            ...
            if i == 1 and self.forecast:
                ...
                self._text(
                    ...
                    f"в том числе «Планирование: прогноз на будущий период» – стр. {fc_pg:02d}",
    ...
    def s10_methodology(self):
        ...
        self._section_intro(
            slide, 3, "Методология",
            "Байесовская MMM с отложенным эффектом и Hill-насыщением – прозрачная модель с правдоподобными диапазонами",  # П8-1
        )

        _title_text = (
            "OLS MMM с отложенным эффектом (adstock) + Hill-насыщением (closed-form + bootstrap-диапазон)"  # П8-2
            if self.is_ols
    ...
    def s_forecast_plan(self):
        ...
        # слайд – «Прогноз на будущий период»; заголовок связывает оба имени.
        self._action_title(
            slide, "Планирование: прогноз на будущий период",
            show_lime=True, y=0.80, height=0.80,
        )
