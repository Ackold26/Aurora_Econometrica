...
Источники (разведка 13.09.2026, см. Projects/PULSE_s45_macro.md):
  ...
  курс_доллара / курс_евро / курс_юаня
    ...
    windows-1251, десятичная запятая. Официальный курс устанавливается по
    рабочим дням (не каждый календарный день). История – с 1992 г.
  ...
  ключевая_ставка
    ...
    задокументирован независимо (1С-интеграции, инфостарт и т.д.), но
    ТОЧНУЮ схему ответа вживую проверить не удалось (тот же сетевой блок).
    По независимой документации ответ – ASP.NET DataSet-diffgram с
    повторяющимся <KR><DT>...</DT><Rate>...</Rate></KR>. Парсер ниже это
    предполагает и падает с полным ответом в тексте ошибки, если не найдёт
    ни одной пары – первый боевой прогон покажет реальную схему, вместо
    того чтобы молча вернуть пустоту или неверные числа. Находка №2 –
    нужна сверка владельцем/первым боевым прогоном.
...
Приведение к периоду (неделя/месяц) и обоснование выбора:
  Курс и ключевая ставка – СРЕДНЕЕ доступных официальных дневных значений
  внутри периода (не «конец периода»). ЦБ фиксирует курс по рабочим дням, а
  ставка действует непрерывно между решениями Совета директоров; среднее
  показывает условия, в которых бизнес принимал решения на протяжении всего
  периода, тогда как «конец периода» – случайный однодневный снимок,
  который на валютном рынке легко исказить разовым скачком. Для ставки
  среднее к тому же честно отражает смену ставки в середине периода (была
  18%, стала 17% – среднее покажет промежуточное значение, а не одну из
  границ).
  ...
  индексов»). Понедельно – forward-fill месячного значения (см. находку №3).
...
def aggregate_daily_mean(
    ...
    модуля – обоснование выбора «среднее», а не «конец периода»)."""
...
def fetch_currency_result(code: str, name: str, date_from: dt.date, date_to: dt.date) -> FetchResult:
    res = FetchResult(name)
    try:
        daily = fetch_cbr_currency_daily(code, date_from, date_to)
        res.last_date = daily[-1][0]
        weeks = full_week_sequence(date_from, date_to)
        months = full_month_sequence(date_from, date_to)
        res.weekly_level = aggregate_daily_mean(daily, iso_week_key)
...
def fetch_key_rate_result(name: str, date_from: dt.date, date_to: dt.date) -> FetchResult:
    res = FetchResult(name)
    try:
        daily = fetch_cbr_key_rate_daily(date_from, date_to)
        res.last_date = daily[-1][0]
        weeks = full_week_sequence(date_from, date_to)
        months = full_month_sequence(date_from, date_to)
        res.weekly_level = aggregate_daily_mean(daily, iso_week_key)
...
def write_csv(
    ...
    for col, _name, _unit in INDICATORS:
        header += [f"{col}_уровень", f"{col}_изменение_pct"]
...
def write_readme(path: Path, results: dict[str, FetchResult], date_from: dt.date, date_to: dt.date) -> None:
    lines = [
...
def main(argv: list[str] | None = None) -> int:
    ...
    results: dict[str, FetchResult] = {}
    results["курс_доллара"] = fetch_currency_result("usd", "Курс доллара", date_from, date_to)
    results["курс_евро"] = fetch_currency_result("eur", "Курс евро", date_from, date_to)
    results["курс_юаня"] = fetch_currency_result("cny", "Курс юаня", date_from, date_to)
    results["ключевая_ставка"] = fetch_key_rate_result("Ключевая ставка", date_from, date_to)
    ...
    write_readme(args.out_dir / "README_macro_series.md", results, date_from, date_to)
