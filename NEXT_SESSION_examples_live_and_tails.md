...
> 🛑 УСТАРЕЛ — ЗАМЕЩЁН `NEXT_SESSION_econ_live_recs_and_C.md` (2026-07-05, после
> 4 кругов аудита). Читать ТОТ файл. Здесь — только исторический контекст Блоков A/B/C.

> ⚠️ ОБНОВЛЕНИЕ 2026-07-05 (после написания промта): **Блок B ВЫПОЛНЕН + аудит
> сессии** (коммиты 73d8e55..на origin): B2 канарейка `test_role_detectors_parity.py`
> (кусается — доказано мутацией Д-1) · B1 справка data-preparation под пары
> (+ аудит-фиксы: реальные значения превью, Ratio с экрана программы, честность
> про heatmap-подсветку пар, .tx-ex стиль) · B3 = verify_existing_impl (Фаза Б
> УЖЕ была готова @7cc4212; добавлен прямой тест breakout-полосы «Категория»
> `test_decomposition_category_band.py`, кусается — доказано мутацией).
> Реестр — в `AUTONOMOUS_WORK_STATE_SEASONALITY.md`. **Актуальны только Блоки A и C.**
> ОБНОВЛЕНИЕ №2 (позже в тот же день): 5 рекомендаций аудита выполнены по решениям
> Антона — движковое ПОЧИНЕНО: календарь праздников v2.1/v2.2 (окна подготовки ДО
> события / распродажи ОТ старта · fraction-доля периода · дедуп имён · миграция
> binary_point для старых моделей), бейдж пар на heatmap + инсайт-слое, ночной гейт
> с трип-проволокой, гейт ссылок справки. Второй аудит — тоже в реестре.
> К живой приёмке (Блок A) добавились: полоса «Праздники» на месячном фите теперь
> содержательная · вид бейджа пар на карте корреляций и в инсайтах.
