...
## Волна MC — multi-client прогон отчётного пути (правило памяти: ≥2 клиента)
| # | Задача | Статус |
|---|---|---|
| MC-1 | Венарус: validate→train→decompose→optimize→backtest→PPTX, 0 маркеров | 🔶 в работе: найден и ИСПРАВЛЕН F-MC-1; повторный прогон ЗАБЛОКИРОВАН EPERM (шеллы не спавнятся) — повторить по возврату среды: убить сайдкар 8911 (старый код!), поднять свежий, `python tmp\probe_multiclient_deck.py "D:\Docs\Aurora_Ai\TestData\Econometrica\Venarus_MMM_dataset.xlsx" venarus` |
| MC-2 | MMX 2021-2025 (длинный ряд): тот же путь (`...\MMX_2021-2025_source.xlsx` mmx) | ⏳ TODO |
| MC-3 | MATH_REFERENCE-дополнение (E1 предиктивный интервал, E3 перекрытие CI, E2 калибровка-likelihood) | ⏳ TODO (структура doc изучена: вставка новым разделом «Trust loop E1-E4» перед Literature) |
