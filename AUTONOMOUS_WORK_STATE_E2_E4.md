...
| MC-5 | HTML-версии блоков петли доверия | ✅ секция «Петля доверия» (render_trust_loop: 4 условных под-блока — витрина E1 с таблицей окон · «что изменилось» E3 с CI и вердиктами · калибровка E2 с честным расхождением · сбывшиеся E4 с «не сбылось») после timeline; TOC-пункт условный (пусто → секции и пункта НЕТ); стили в layout.css; швы build_html(+3 параметра) + /export/html читает артефакты с диска как PPTX. Тесты tools/test_html_trust_loop.py 3/3 (bare без секции/TOC · full все 4 блока · 0 wireframe) + регресс B1 8. Грабля: сырая строка лейбла живёт в инлайновом strings-JSON для JS — контракт проверяет рендер и TOC, не сырые строки |
