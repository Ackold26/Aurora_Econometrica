ЯЗЫК ОТВЕТА: РУССКИЙ. Все комментарии, выводы, рекомендации, статусы — на русском языке. НЕ использовать английский (кроме терминов ROI, CPM, GRP).

Подготовь исправленную версию NDA из inbox с правками в режиме рецензирования Word (Track Changes).

## Порядок работы

1. Прочитай NDA из inbox (docx или pdf)
2. Проанализируй все параметры: объём конфиденциальной информации, исключения, срок действия, санкции, возврат информации, юрисдикция
3. Сформулируй исправленную редакцию для каждого спорного пункта
4. Создай DOCX с Track Changes, используя python-redlines:

```bash
pip install python-redlines python-docx 2>/dev/null
```

5. Напиши и выполни Python-скрипт:

```python
from redlines import Redlines
from docx import Document

# 1. Прочитай оригинальный NDA
original = Document("inbox/[имя файла]")
original_text = "\n".join([p.text for p in original.paragraphs])

# 2. Создай исправленную версию
modified_text = original_text  # Здесь подставь исправленный текст

# 3. Сгенерируй redline-версию
diff = Redlines(original_text, modified_text)
diff.output_to_docx("exports/nda-redline-[дата].docx")
```

6. Дополнительно сохрани протокол разногласий:

| # | Пункт | Текущая редакция | Предлагаемая редакция | Правовое обоснование |
|---|-------|-----------------|----------------------|---------------------|

## Результат

В exports/ должны появиться два файла:
- `nda-redline-[дата].docx` — NDA с правками в режиме рецензирования (Track Changes)
- `nda-protocol-[дата].docx` — текстовый протокол разногласий

Если Python не установлен, выведи сообщение: "Для генерации DOCX с Track Changes требуется Python 3. Установите с https://www.python.org/downloads/ и перезапустите команду." — и создай только текстовый протокол.
