...
1. Проверь, что данные валидированы (`/mmm-prepare`). Если нет – сначала провести валидацию. Inbox пуст – сообщи, что нужен файл с данными. Несколько файлов – возьми последний по дате изменения и назови его в ответе. Файл повреждён/нечитаем – сообщи какой и почему, не выдумывай содержимое
...
4. Сгенерируй Python-скрипт `exports/scripts/mmm_model.py`:
   ...
   - **Проверка остаточной стационарности** (после вычитания тренда/сезонности Prophet): прогони ADF/KPSS на остаточном ряде KPI и медиа. Prophet-тренд – гладкая детерминированная кривая, не тест на unit root; остаток может остаться интегрированным. Регрессия двух нестационарных рядов даёт ложно высокий R² без причинной связи (Neusser, spurious regression). Остаток нестационарен (unit root не отвергнут) – пометь `[ОГРАНИЧЕНИЕ: остаточная нестационарность, риск ложной регрессии]`; если ряды коинтегрированы – регрессия на уровнях осмысленна, иначе рассмотреть спецификацию в разностях
