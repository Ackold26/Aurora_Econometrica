...
### Т5б. Сезонная волна в прогнозах (= У1 ниже, УТВЕРЖДЕНО Антоном; методологическая находка аудита М-1)
...
## ✅ УТВЕРЖДЁННЫЕ УЛУЧШЕНИЯ ИЗ АУДИТА (Антон принял все 5, 2026-07-04)

Пять предложений по стабильности/надёжности/честности из аудита сессии
(`docs/audits/AUDIT_SESSION_WORK_2026_07_04.md`). Все УТВЕРЖДЕНЫ — реализовать в этой
сессии вместе с декомпозицией. Порядок по приоритету указан.

### У1. Сезонная волна в прогнозах (ВЫСОКИЙ — делать с декомпозицией; = Т5б выше)
Детали и точки правки — в Т5б. Суть: predict_scenario должен учитывать
детерминированную будущую сезонность (`Σ β_fourier·fourier(t_future)·y_std` к
baseline), иначе прогнозы на неполный цикл смещены. Затрагивает scenario/промисы E4/
drift_check/goal-seek. Тест на класс + сверка промиса-цикла (`tmp/probe_e4_live_cycle.py`).
Методологически: без этого «база+сезонность» в декомпозиции (историческая) и прогноз
(будущий) рассогласованы — клиент увидит сезон в разложении, но не в прогнозе.

### У2. Канарейка паритета фронт↔серверные схемы (СРЕДНИЙ — защита от F-AUD-1)
Автотест, ловящий класс «строгая схема молча роняет поле» ДО боя. F-AUD-1 (тумблер
праздников был декорацией) вскрылся только сквозным аудитом — нужна автоматика.
Реализация (python-тест, `tools/test_frontend_schema_parity.py`):
- Список ключей, которые фронт шлёт в train: извлечь из `src/lib/train-config.js`
  `buildTrainConfig` (регэксп по ключам возвращаемого объекта ИЛИ поддерживаемый вручную
  список-якорь с комментарием в обоих файлах).
- Поля серверных схем: `server.TrainRequest.model_fields.keys()` +
  `TrainStartRequest.model_fields.keys()` (Pydantic v2 интроспекция).
- Ассерт: КАЖДЫЙ фронт-ключ ∈ полям схемы (иначе Pydantic отбросит молча → фича-декорация).
  Фейл с именем потерянного поля. Кросс-язык паритет — аналог mirror JS↔Rust из методов.
- Якорь-комментарии в train-config.js и обеих схемах: «при добавлении флага — обнови обе
  стороны + этот тест».

### У3. Числовой гейт ролей на Валидации (СРЕДНИЙ — системно закрывает F-AUD-5)
Не предлагать роль media/control НЕЧИСЛОВЫМ столбцам (они входят в X численно →
astype(float) упадёт). F-AUD-5 закрыт точечно (условие ТЕМА+ОБЪЁМ), но класс шире:
любой текстовый атрибут-классификатор. Реализация:
- В `data_preview`/автоклассификации (где есть доступ к df): после detect_column_role
  по ИМЕНИ проверить ПАРСИБЕЛЬНОСТЬ колонки в число (не сырой dtype — «3 836 962 ₽»
  строка, но парсится; использовать логику `_parse_rub`/pd.to_numeric с coerce, порог
  ≥~80% непустых парсятся). Нечисловая + роль media/control → понизить до 'unused'/
  'text' с подсказкой «столбец текстовый, не может быть предиктором».
- Осторожно: не задеть легитимные money-строки с разделителями/₽ и count-колонки.
  Тест: money-строка «3,836,962 ₽» остаётся control-годной; чистый текст «Категория А/Б» — нет.

### У4. Детектор сезонности — предпочитать длинный период (СРЕДНИЙ — методология М-2)
На медленной годовой волне короткий лаг-алиас (напр. lag=4 при годовом периоде)
конкурирует по автокорреляции и может выиграть → 2 лишних параметра, слабая
аппроксимация. Реализация в `utils/forecast_validation.py::detect_seasonality`
(строка ~187-195, сейчас `top = max(pos, key=autocorr)`):
- Среди кандидатов, прошедших порог значимости (`pos`), предпочесть НАИБОЛЬШИЙ период,
  если его autocorr не сильно ниже максимума (эвристика: autocorr ≥ k·max_autocorr,
  k≈0.8; иначе fallback на max-autocorr). Осмысленные периоды (годовой 52/12, полугодовой
  26/6) выигрывают у мелких алиасов.
- ⚠️ Меняет поведение автосезонности (какой период инжектится) → прогнать ВСЕ
  fourier-тесты (22+7) + бэктест-зонд MMX + не сломать существующие тесты detect_seasonality
  (grep тестов на detect_seasonality). Гейт значимости Bartlett не трогать.

### У5. Ночной полный gate на реальных данных (НИЗКИЙ — инфра против F-AUD-6)
F-AUD-6: тест-инфра тихо протухла (путь Desktop переехал, 18 тестов красные, gate
маскировал регрессы). Пути теперь через `AURORA_TESTDATA_DIR`. Настроить регулярный
прогон полного gate на реальных данных:
- Скрипт-обёртка (`tools/nightly_full_gate.ps1` или .sh): экспорт AURORA_TESTDATA_DIR →
  `pytest tools/ -q` → лог с датой → при exit≠0 алерт (лог-файл/Telegram по инфра-каналу
  Маши Небесной, если доступен).
- Регистрация в Windows Task Scheduler (паттерн «Aurora Memory Reindex») ЛИБО регламент
  «запускать перед каждым релизом». Это инфра, не код продукта — можно вынести отдельным
  малым коммитом.
...
2. Прочитать durable-реестр + decomposer (точные строки выше) + отчёт аудита
   `docs/audits/AUDIT_SESSION_WORK_2026_07_04.md` (контекст У1-У5).
3. **Порядок работ (декомпозиция + утверждённые улучшения переплетены):**
   - Т1 (backend-ядро) батчами: classify → агрегация → BREAKOUT → % к базе → тесты → зонд MMX → коммит.
   - **У4** (детектор длинного периода) — сделать РАНО, до глубокого доказательства декомпозиции:
     влияет на то, какой период инжектится, значит и на величину полосы «Сезонность».
   - Т2 (верхний уровень 4 групп) → Т3 (UI) → Т4 (отчёты).
   - **У1/Т5б** (сезонность в прогнозах) — вместе с декомпозицией: историческое разложение
     и прогноз должны быть согласованы.
   - Т5 (доказательство мульти-клиент + ADR).
   - **У3** (числовой гейт ролей) + **У2** (канарейка паритета схем) — надёжность, средний приоритет.
   - Т6 (хвосты: тумблеры/подсказки) + **У5** (ночной gate, отдельный малый коммит, низкий приоритет).
Каждое У — с тестом на класс; У4 требует прогона ВСЕХ fourier-тестов (меняет период).
