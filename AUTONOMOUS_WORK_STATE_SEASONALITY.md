...
## ✅ R3 INSIGHTS-КАНАРЕЙКА ШИРЕ ВЫПОЛНЕН (2026-07-05, та же сессия; ПЕРВАЯ задача по мандату «кодинг → субагент lower-power»)
Recon мой (промт устарел: реально покрыты validate/model/optimize/decompose/report
частично; СОВСЕМ голые — 6: importInsights, validateKpiInsights, validateRoles,
validateMetrics, validateConfirm, modelPreTrainingInsights). ТЗ субагенту sonnet:
1 новый файл, negative-first кейсы-убийцы (F-007 stale unit_cost, early-return
0 media, рассинхрон enabledMediaNames, границы 24/52/104/ratio<4), якорные
подстроки (не дословные), исходник НЕ трогать. **Субагент выдал:**
`src/lib/__tests__/insights-uncovered-functions.test.js`, 50 тестов, регресс
соседей 0, исходники нетронуты, багов не нашёл (3 «находки» = поведение по коду).
**Моя приёмка (лично):** ревью файла построчно (якоря сверила с исходником,
ложно-зелёных от joinText(tip) нет) · свой прогон 77 (50+соседи) · **2 мутации
исходника** — М1 текст «Мало данных»→красный B-тест; М2 логика F-007
(inputReady=true)→красный ровно F-007-тест; обе откачены, diff исходника пуст
байт-в-байт · соседний прогон 132 зелёных. ВЕРДИКТ: принято без доработки.
Пробелы некритичные (слайс «3 из 5» KPI, «~N медиа») — осознанно не гоняла
субагента ради них. Гейт: vitest полный (в фоне на момент записи; коммит после
сводки).

## ✅ R1 UNDERSCORE-РЕФАКТОР ДЕТЕКТОРОВ ВЫПОЛНЕН (2026-07-05, сессия live_recs_and_C, Антон выбрал «R1 автономно»)
Метод — корпус-оракул (совет Антона той же сессии: находки в РАСПРЕДЕЛЕНИИ входов,
не в коде): зонд `tmp/r1_corpus_probe.py` (26 позитивных пробельных форм + 19
коварных соседей) через живые функции ДО правки. **Baseline вскрыл:** 11/26
validator-MISS; 🔴 латентный баг — «usd rub»/«eur rub» classify ловил голым `usd`
из MONETARY → **media с ROI** (не unknown!); «sign up»/«app install» — рассинхрон
Д-1 (знал только classify). Сюрприз-опровержение: «share of voice»→unused (derived)
— корректно, моё ожидание control было неверным (убрала из позитива).
- **Фикс classify (`column_detection.py::_sep_pattern`):** внутренний `_`
  составного токена → separator-класс `[_\s\-]` (одна строка — чокпоинт, все
  compound-паттерны получили пробел/дефис-формы разом; JS-зеркало получает
  автоматически через export_patterns_as_json → фронт реконструирует RegExp).
- **Фикс validator (плоский substring, БЕЗ голого фолбэка у курс_*/fx/exchange/
  usd_rub):** тройки underscore+пробел+дефис (прецедент 'тв '/'тв_'/'тв-') для
  fx_rate/exchange_rate/usd_rub/eur_rub/курс_{рубля,доллара,евро}; KPI + signup/
  sign up/sign-up/app install/app-install. Голые 'курс'/'install' НЕ добавлены
  (дискурс/экскурсия/installment — анти-корпус держит).
- **Канарейка (`test_client_names_detector_canary.py`): 170** (было 166 всего в
  файле до правки — теперь строгий корпус + 13 пробельных/дефисных форм;
  `test_currency_rate_is_macro_not_media` — регресс латентного бага; класс
  `TestSeparatorFlexNoFalsePositive` — 9 коварных соседей).
  Дефис-дыру validator нашла КАНАРЕЙКА на моём же фиксе (курс-доллара красный) —
  рассинхрон Д-1 чуть не узаконила, закрыт тройками.
- **Мутации доказаны:** A (снят replace в _sep_pattern) → 4 красных; B (снята
  'курс доллара' из validator) → 2 красных; обе откачены обратной правкой.
- **Гейт (живой прогон nightly_full_gate, вердикт по сводке, не exit-коду):**
  python **2272 passed, 6 skipped** (+51 к 2221) · vitest **875** · svelte-check
  **0 errors**. Коммит узким pathspec (3 файла), зонд остался в tmp/ (не коммитится).
...
## ✅ АУДИТ РАБОТЫ БЛОКА B ПО ЗАПРОСУ АНТОНА (2026-07-05, второй круг, свежий движок)
Метод: 12 гипотез классов → зонды (не рассуждение). **7 находок, все исправлены:**
- **A-1 🔴 выдуманные ячейки превью FMCG:** строка 2022-02-28 несла tv_spend
  «21 400 000»/tv_trp «112», реально **169 297 ₽ / 0.7 TRP** (тёмный месяц
  флайтинга). Исправлено на реальные + пункт-пояснение про флайтинг (контраст
  полезен модели). Урок: превью «реального файла» сверять ячейка-в-ячейку.
- **A-4 🔴 справка противоречила экрану:** фраза «программа не считает пары
  мультиколлинеарностью» — а CorrelationHeatmap показывает ВСЕ 4 пары с r=0.99
  и текстом «Мультиколлинеарность» (high_correlations не фильтрует пары).
  Переписано честно: подсветка ожидаема, не блокирует, в модель идёт одна колонка.
- **A-2/A-3 🟠 неточные float превью:** fmcg price_index 1.00/0.99→1.03/0.98;
  realestate macro_cpi 1.00/1.01→1.01/1.02, competitor 22→22.5 (зонд без округления).
- **A-5 🟠 фраза про Black Friday:** «остальные праздники добавляет сама (не
  дублируй)» — а авто-календарь СОДЕРЖИТ black_friday, и пример сам «дублирует»
  (дедуп по имени не срабатывает: авто `holiday_black_friday` ≠ ручная
  `holiday_blackfriday`). Переформулировано: свой флаг оправдан для категорийно-
  критичного события. Движковая часть → рекомендации (вне скоупа).
- **H14 🟡 разночтение развязки:** «программа сама возьмёт» (р.8) vs «выбираешь»
  (р.6) → унифицировано с фактом: по умолчанию бюджет ₽, выбор на подшаге
  «Метрики каналов» (имя сверено с ValidateStepV13:961).
- **H16 🟡 класс-декорация:** `.tx-ex` был определён только в скоупе `.tax-card`,
  мои 3 абзаца вне него рендерились без стиля. Добавлено общее правило.
- **H25 🟡 NEXT_SESSION промт** не знал о выполнении Блока B → добавлена шапка-
  обновление (актуальны только A и C + известное движковое).
**Мутационные доказательства обоих новых тестов:** B2-канарейка — полная мутация
Д-1 (все 4 токена из MEDIA_PATTERNS) → 2 failed на apteka_contacts; частичная
(только apteka) корректно НЕ тревожит ('contact' держит role=media — рассинхрона
нет). B3 — убран 'category' из _BREAKOUT_TYPES → 2 failed. Оба восстановлены
бит-в-бит (git diff пуст).
- **A-8 🔴 полный python-гейт был красным (pre-existing хвост ветки):** запуск
  ПОЛНОГО pytest tools/ (метод-пробел: сессии гоняли целевые списки) вскрыл
  9 failed в `test_priors_calibration.py` — тест Phase E2 не мигрирован при
  пересборке генератора примеров (@cbc20ee): старые per-channel ключи GT
  (`tv_decay`→`decay['tv']`), старые колонки (`ooh_trp`/`apteka_ooh_contacts`),
  удалённый `holiday_newyear`, и трансформ фич (z-normalize→adstock→hill γ0.6)
  ≠ трансформу генератора (adstock на сыром→/mean→hill γ0.9). Мигрирован
  полностью: physical-носители пар, dict-ключи, сезонная cos-волна контролом
  (сезон теперь baked в базу), holiday-тест переехал на retail blackfriday,
  sigma_adequacy расширен (macro/promo/category, 5→9 кейсов). 9 failed →
  **11 passed** без ослабления порогов (R²>0.40, знаки) — подтверждение, что
  фичи совпали с истиной генератора. ⚠️ Урок: exit 0 пайпа шёл от tail —
  вердикт только по хвосту вывода pytest, не по коду цепочки.
**Движковая находка (НЕ чинилась, хирургичность):**
авто-праздники дневных окон на месячной грануляции с датой конца месяца флаки
(auto-BF 1 только в 2/4 ноябрей — когда окно цепляет 30-е) и 6/12 вечно нулевые;
дедуп с ручными holiday-колонками — по точному имени. Кандидат в задачу движка.

## ✅ АУДИТ №3 — НЕАУДИРОВАННАЯ РАБОТА (рекомендации 1-5 + v2.2 + priors-миграция; 2026-07-05)
14 гипотез → зонды. **2 находки исправлены:**
- **B1 🔴 инсайт-слой не знал о парах:** `insights-rules.js::validateInsights`
  (строка ~650) строил СВОЁ предупреждение «Мультиколлинеарность» из
  result.correlations — на fmcg-примере InsightsPanel пугал «tv_spend ↔ tv_trp»
  warning'ом, пока heatmap рядом говорил «ожидаемо» (бейдж №2 не доехал до
  второго UI-слоя). FIX: декларированные пары → отдельная info-строка
  («N пар "бюджет + натуральная метрика" — ожидаемо», tip про одну колонку в
  модели), кросс-канальные остаются warning. +4 vitest
  (insights-collinearity-pairs.test.js: пара→info, кросс→warning, смесь, порог).
- **F 🟠 шапка NEXT_SESSION снова врала:** «движковое НЕ чинилось» — после
  выполнения рекомендаций календарь починен; шапка обновлена (v2.1/v2.2 +
  что добавилось к живой приёмке).
**Доказательства не-декоративности:** живой прогон nightly_full_gate.ps1 после
правки — [OK] python 2042 · vitest 866 · svelte 0 errors (сводки парсятся,
трип-проволока в деле); мутация GT (competitor_coef -0.16→+0.16) → priors-тест
failed → откат passed (мигрированный тест видит данные, не воздух).
**Чистые зонды:** backtest/scenario/drift праздники не потребляют (fraction
безопасен) · справка: кавычки двойные, src=econ-nav.js покрыт гейтом ссылок ·
BF-период (до 13 дек) не наезжает на NY-preshop (с 15 дек) · позиции
control_coefs во всех мигрированных тестах сверены.
**Edge-вопросы к Антону (не чинились без решения):** дедуп не гасит юзерскую
BF-колонку БЕЗ префикса holiday_ («blackfriday» — норм-имя не совпадает) ·
длины sale-окон (ЧП 14 дн = нижняя граница «2-3 недель», CM 7 дн = Cyber Week
— короче буквы решения).

## ✅ АУДИТ №4 — дедуп/канарейка/insights-слой (2026-07-05, Fable-круг)
11 гипотез → зонды. **Главная находка Д2 🔴🔴 (исправлена): мой углублённый дедуп
создавал ПОЛНУЮ потерю контроля** — сценарий-мотиватор (клиентская `black_friday`
без префикса) падал в unknown→unused у ОБОИХ детекторов (в модель не шёл), а
дедуп по df.columns гасил авто-инжект → событие без контроля вовсе (OVB молча;
хуже дубля, который дедуп чинил). Зонд: 5 из 14 клиентских форм в unknown у
обоих; бонус — `8_марта`/`23_февраля`/`каникулы` знал только classify (ещё
Д-1-рассинхроны), `день_россии`/`день_победы` classify давал ЛОЖНУЮ date
(generic «день» раньше специфичного имени события).
**FIX (SSOT, вся цепочка доезда):** `is_holiday_like_name` в календаре (те же
алиасы + анти-ложная защита) → оба детектора: classify→'holiday' (ДО
generic-date), validator→'control' (до частотного скоринга). Каскад согласован:
role control → control_cols → modeler kind holiday prior 0.0 → decomposer
полоса «Праздники»→БАЗА. Фронт-каскада нет (registry фронта — только
detectChannelUnitType, holiday-формы не задевает).
**Д1 🟠 (исправлена):** голый алиас 'валентин' ловил имя человека
(«Валентина_план») → сужен до событийных форм (валентинк/деньвалентина/14февраля).
Зонд после фикса: 15 форм → holiday/control согласованно, 0 ложных на анти-корпусе.
Тесты: канарейка += TestHolidayClientForms (17 форм ×2 детектора + 8 анти-ложных);
дедуп-корпус += валентина_план. Детекторные сьюты 792 → 199 новых прогонов зелёные.
Чистые зонды: фронт-паритет classifier-patterns (не задет) · '9_мая' был holiday
и до фикса · экспорт-registry не менялся.

## ✅ ПОСТ-АУДИТ: дедуп углублён + канарейка клиентских имён + шире insights (2026-07-05)
Антон: №1/3/5 «ок» (уже сделано), №2 «продумай глубоко и сделай как рекомендуешь»,
+ две мои рекомендации. Выполнено:
- **№2 Дедуп-щель УГЛУБЛЁН** (`holiday_calendar_ru.py`): клиент не знает конвенцию
  префикса `holiday_` → колонки `black_friday`/`8_марта`/`valentine`/`день_россии`
  теперь опознаются курируемым whitelist `_HOLIDAY_ALIASES` (RU/EN синонимы +
  дата-формы). 🔴 Принцип: ЛОЖНОЕ гашение (потеря контроля → OVB) опаснее
  пропущенного → двойная защита: (1) whitelist только специфичных длинных ядер
  (нет голых may/russia/unity/march), (2) гейт `_MIN_SUBSTRING_ALIAS`=5 (короткий
  безцифровой алиас — только точное равенство), (3) точное авто-имя покрывает РОВНО
  его. Зонд: 18 форм гасятся, **0 ложных на 37 реальных колонках** (mayonnaise/
  russian_market/community/marchmadness чисты). Доказано мутацией: длинное `sales`
  в алиасы → 6 ложных гашений (тест кусается); короткое `may` гейт перехватил.
  +тесты (без-префикса ×20, анти-ложное ×35, точное→одно, newyear→оба, ssot-алиасы).
- **(б) Канарейка клиентских имён** (`tools/test_client_names_detector_canary.py`,
  77): 34 реалистичных имени (RU/EN, пробелы, запятые, единицы) через ОБА детектора.
  🔴 Вскрыла боем 2 рассинхрона класса Д-1 (ИСПРАВЛЕНЫ в validator): `GMV` знал
  classify, не validator (→«не найден KPI»); русское «Период» знал classify, validator
  только англ (→«не найдена дата»). Оба — блокеры первого впечатления.
- **(а) Шире insights** (`src/tests/validate-insights-status.test.js`, 9): ветки
  статуса validateInsights (нет KPI / 2 KPI / нет медиа / Ratio<2 / Ratio 2-4 /
  готово / effectiveStatus из ролей не stale-backend) — 2000+ строк правил впервые
  под прямым юнитом клиент-facing функции.
**🟠 Рекомендация Антону (НЕ чинила — широкий рефактор):** underscore-в-паттернах
обоих детекторов ломает распознавание клиентской формы с ПРОБЕЛОМ («индекс цен»,
«курс доллара» → unknown; underscore-форма работает). Затрагивает SIGNED_PRICE/
SIGNED_MACRO/validator CONTROL. Частый кейс (клиентские файлы с пробелами в именах).

## ✅ КАЛЕНДАРЬ v2.2 — КЛАССЫ ОКОН (уточнение Антона №2, 2026-07-05)
Праздники ≠ распродажи: у праздников активность ДО события (окно подготовки,
2-3 недели — эти окна уже были верны), у РАСПРОДАЖ — С МОМЕНТА СТАРТА и по
завершении (~2-3 недели от старта). Реализовано:
- `window_kind` каждому из 12 событий: preparation (НГ-закупки/Valentine/23фев/
  8мар/back-to-school) · sale_period (ЧП/Cyber Monday/новогодние распродажи) ·
  calendar_period (майские/госвыходные/каникулы).
- Окна распродаж: ЧП 3 дня → **[старт, +13]** (14 дней от посл. пятницы ноября);
  CM 1 день → **[старт, +6]** (Cyber Week). NY-postsale уже был от старта ✓.
- Миграция: binary_point (decompose моделей v2.0/v2.1-до-этого) воспроизводит
  СТАРЫЕ узкие окна через `date_range_v20` — β старых моделей согласованы с их X.
- Зонд на месячных примерах: период размазан ноя→дек пропорционально старту
  (2022: 0.20/0.26 · 2024: 0.07/0.39; CM-2024 старт 2 дек → ноябрь честно 0).
- Тесты 24 (стало): +window_kind классификация 12 событий · sale_period точные
  доли (BF-2022 6/30+8/31, CM 3/30+4/31) · «v2.0-окна живут только в
  binary_point» (daily: legacy 3+1 день, fraction 14+7) · daily-вырождение
  уточнено (равенство режимов — для событий без v20-переопределения).

## ✅ РЕКОМЕНДАЦИИ АУДИТА ПРИНЯТЫ АНТОНОМ И ВЫПОЛНЕНЫ (2026-07-05, все 5)
Решение Антона по №1: моделировать не праздничные дни, а **окно покупательской
подготовки** (~2-3 недели до события — закупки подарков); №2-5 — «ок».
- **№1 Календарь праздников v2.1** (`holiday_calendar_ru.py` + modeler + decomposer):
  (а) принцип окон подготовки закреплён докстрингом (дефиниции уже были
  preshop: НГ 15-31 дек, Valentine 1-14 фев, 23фев 15-23, 8мар 1-8, BTS);
  (б) значение дамми = ДОЛЯ дней периода строки в окне (fraction 0..1) вместо
  точечной даты — на месячных данных 6/12 вечно-нулевых праздников ожили,
  ЧП видна каждый ноябрь (~0.1), дневная грануляция вырождается в старые 0/1;
  (в) семантический дедуп авто-инжекта с ручными колонками по норм-имени
  (holiday_blackfriday гасит holiday_black_friday — двойной учёт закрыт);
  (г) миграция: pickle несёт `holiday_dummies_mode='fraction'`, decomposer
  воспроизводит режим train (старые модели без ключа → binary_point, их β
  согласованы с бинарными X); (д) collinearity-детектор на >0 семантике.
  Тест `tools/test_holiday_calendar_granularity.py` (16): точные доли, инвариант
  начала/конца месяца, недельные доли, daily==binary, legacy-режим воспроизводит
  старый флак, дедуп, collinearity.
- **№4 Комментарий генератора** «BF НЕ в РФ-календаре» (ложь) → честный: авто-
  календарь знает ЧП, но ручная колонка примера гасит авто-инжект (дедуп v2.1).
- **№3 nightly_full_gate.ps1**: механика была честной ($LASTEXITCODE после
  Tee-Object сохраняется от exe — в отличие от bash-пайпа A-8); добавлена
  трип-проволока на обратный класс: exit=0 БЕЗ строки-сводки прогона (упавший
  сбор/проглоченный запуск) → gate FAILED + null-guard кодов. Синтакс-зонд ок.
- **№5 `tools/test_help_links.py`** (29): якоря href="#id" резолвятся, локальные
  файлы ссылок существуют, анти-вакуум ≥10 html. Кусается (мутация якоря → 1 failed).
- **№2 Бейдж пар на карте корреляций**: `channel-pairs.js` += declaredPairKeys/
  isDeclaredPair (пары из labels матрицы — ValidateStep не тронут);
  CorrelationHeatmap: пары НЕ красятся danger (канвас+tooltip), в списке бейдж
  «Пара канала — ожидаемо: в модель уйдёт одна колонка», заголовок разделяет
  реальные пары риска и пары каналов. +5 vitest (857 → 862).
Гейты: vitest **862** (+5) · svelte-check **0 errors** · python ПОЛНЫЙ (tools/ +
sidecar/tests/) **2425 passed · 0 failed** (PYTEST_EXIT честный, не пайп).
