...
## ✅ БЛОК B АВТОНОМКИ (промт `NEXT_SESSION_examples_live_and_tails.md`) ВЫПОЛНЕН (2026-07-05)
Три автономных доработки закрыты, 4 коммита на origin (ветка `feat/econ-e1-backtest`,
73d8e55→285b480). Мандат «максимально автономно»; Блоки A (живая приёмка) и C
(комментарии Антона по примерам) ждут Антона у экрана.
- **B2 канарейка детекторов ролей (73d8e55):** `tools/test_role_detectors_parity.py`
  (53) — для каждой колонки примеров `validator.detect_column_role` сверяется с
  `classify_column` через независимую таблицу-контракт KIND_TO_ROLE. Трип-проволока
  на класс Д-1 (apteka знал один детектор, не знал второй). Зонд: 26 колонок, 0
  рассинхронов сейчас (защита от регресса). Ортогонально ssot-тесту.
- **B1 справка под пары (0504ec7 + fix 285b480):** `data-preparation.html` приведена
  к парной схеме — блок-SSOT «Один канал — двумя колонками (пара)» в разделе 6; 4
  встроенных образца (раздел 10) пересобраны под реальные served-файлы (48 мес,
  реальные колонки с парами + category_sales/weather/promo_indicator/holiday_
  blackfriday/macro_cpi, сверены зондом с header'ами); согласовано сообщение про
  пары в разделах 5/8/12/13/14 (запрет остаётся на показы+клики, пара — исключение).
  🟠 Самоаудит-фикс (285b480): числа Ratio были псевдоточные (48/12=4) → сверены
  зондом validate_data с экраном программы (n_predictors 11/10, ratio 4.4/4.8).
- **B3 Фаза Б хвост (0721606):** verify_existing_impl показал — Фаза Б УЖЕ готова
  (коммит 7cc4212, prior mu 0.3 в modeler / suggest_category в validator / breakout
  в decomposer); промпт ссылался на устаревшую запись «частично начата». Закрыт
  реальный пробел: `tools/test_decomposition_category_band.py` (5) — прямой тест
  breakout-полосы «Категория»→ВНЕШНИЕ на SSOT-функции `build_decomposition_series`
  (её ни один тест не дёргал), тождество INV-50, контраст с competitor→КОНКУРЕНТЫ.
  Кусается (мутацией: убрал 'category' из _BREAKOUT_TYPES → 2 упали; откатил).
- **Самоаудит (4 круга, по совету Антона — новизна слоя):** 1 находка 🟠 (Ratio-
  числа B1) исправлена. Финальный гейт затронутого: **739 passed** (parity 53 +
  category_band 5 + ssot 32 + category_control + validator + decomposer_invariants
  114 + collapse + column_detection). Ветка синхронна с origin (behind=0 весь путь).
