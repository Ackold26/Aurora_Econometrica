...
1. **Фаза 2 хвост (verify→fix):** F-14 (classify_column контрольные priors: edge-cases эвристики, fallback μ=0 — читать modeler:~470-510, вероятно TRADEOFF+лог уже есть); F-16 (нормировка mean train-vs-decompose legacy pickle — вероятно закрыто C1-фиксом+I8-тестами, verify по test_optimizer_real_pickle/pickle_compat); F-18 (видимость conformal-caveat пользователю INV-50 — grep exchangeability/jackknife в UI/отчётах; вероятно fix=подпись в отчёте OLS-пути); F-19 (kpi_unit_cost константность во времени — TRADEOFF-doc одной строкой в MATH_REFERENCE); F-06 (sensitivity ±20% — честная ПОДПИСЬ «стресс-тест ±20%», не «неопределённость»: SensitivityTornado.svelte + doc; posterior-вариант → OPP).
2. **Фаза 2 зонд-канарейки:** R²/MAPE/ROI из decompose на synthetic-фикстуре сверить прямым пересчётом (быстрый скрипт по образцу probe, tmp/).
