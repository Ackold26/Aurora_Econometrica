 ...
 *
 * 🔴 Вторая половина защиты (09.08) — не рецепт сборки, а готовый артефакт:
 * `tools/check-installer-edition.mjs` проверяет уже собранный бинарник побайтово и потому
 * ловит и сборку в обход этого скрипта (ручной `tauri build --config …`), которую сам этот
 * гейт не видит.
 */
...
/**
 * Путь к сторожу доставки общего слоя (`aurora-meta/tools/check_crate_consumers.py`).
 *
 * Ищется подъёмом по каталогам от корня дерева, а не относительным путём: рабочие
 * деревья продуктов лежат на разной глубине (`Dev/<продукт>` и `Dev/_wt_*`), и
 * жёсткая относительная ссылка сломалась бы у первого же дерева другой глубины —
 * а сломанный поиск сторожа даёт ровно ту тишину, ради которой сторож и заведён.
 * Возвращает путь или `null`.
 */
export function findDeliveryGuard(startDir) {
  let dir = resolve(startDir);
  for (let depth = 0; depth < 8; depth += 1) {
    const candidate = join(dir, 'aurora-meta', 'tools', 'check_crate_consumers.py');
...
/**
 * 🔴 ЧЕТВЁРТАЯ проверка метки — та, которой тут не было (поручение владельца 30.08.2026).
 *
 * Три прежние проверки отвечают на вопрос «то, что заявлено, взято корректно»:
 * метка существует в удалённом репозитории, состав контракта на ней тот самый,
 * сборка взяла именно её запись. Ни одна не отвечает на вопрос «то, что заявлено,
 * ещё актуально». Замер 30.08.2026: метка `aurora_gateway-v0.6.5` простояла
 * 22 дня, за это время в основную линию доехали ПЯТЬ записей клиентской части
 * общего слоя, и облачная сборка всё это время проходила молча и зелено —
 * потому что метка формально исправна. «Зелёная приёмка» и «старый общий слой»
 * были неразличимы на глаз.
 *
 * Сторож живёт в `aurora-meta` (синхронизируется между машинами через git), а не
 * копией в каждом продукте: копия правила неизбежно разъезжается с оригиналом
 * (класс CPD-14). Здесь — только вызов.
 *
 * 🔴 Отсутствие сторожа — ОТКАЗ, а не тихий пропуск. Пустой сторож зелен всегда
 * и не значит ничего; молчание при пропавшем сторожe — тот же класс беды, ради
 * которого он написан (INV-137).
 *
 * Область ответственности сузена до ЭТОГО дерева (`--tree`): чужой дрейф меток
 * показывается, но сборку не валит — иначе первая же ложная тревога по чужому
 * продукту научила бы обходить проверку вместо того, чтобы её чинить.
 *
 * 🔴 `release` — порог необратимости для ВТОРОГО измерения сторожа (отставание
 * основной линии продукта от рабочей). Работа в незакрытой ветке норма, и
 * краснеть на неё нельзя: сторожа отключат в первый же день. Но `--check` и
 * `--test` никому ничего не отправляют, а сборка установщика делает файл,
...
export function checkShipGate(startDir = ROOT, { release = false } = {}) {
  ...
  // 🔴 Проверка целостности (правка после внешнего аудита 19.09, High): раньше
  // существование файла подтверждалось только `existsSync` — гейт, который
  // подменили (например, тело main() заменили на `process.exit(0)`), сам
  // отчитался бы, что подмены нет. Сверяем рабочую копию с версией, которую
  // git знает как последнюю ЗАПИСАННУЮ — расхождение останавливает сборку
  // ДО запуска гейта, а не полагается на то, что подменённый гейт сам себя
  // разоблачит. Это не защита от решительного взломщика с правом коммита
  // (тот подменит и записанную версию), а страховка от случайной порчи файла
  // и от подмены, не сопровождённой коммитом.
  const committed = spawnSync('git', ['show', 'HEAD:tools/ship-gate.mjs'], { cwd: startDir, encoding: 'utf8' });
  if (!committed.error && committed.status === 0) {
    const onDisk = readFileSync(gate, 'utf8');
    if (onDisk !== committed.stdout) {
      fail(
        'гейт поставки на диске расходится с последней записанной в git версией',
        'tools/ship-gate.mjs изменён без коммита — либо это ваша незакоммиченная правка гейта (закоммитьте её осознанно), либо файл повреждён/подменён. Сборка не продолжается вслепую в обоих случаях.',
      );
    }
  }
  // Если `git show` не нашёл файл в HEAD (самый первый прогон до первого коммита
  // гейта) — целостность сверить не с чем, это ожидаемо один раз при внедрении,
  // не гасим сборку молча, но и не требуем невозможного.
  const run = spawnSync(process.execPath, [gate], { cwd: startDir, stdio: 'inherit' });
  if (run.error || run.status !== 0) {
    fail(
      'гейт поставки не пройден (см. вывод выше)',
      'AURORA_SHIP_OVERRIDE=1 + AURORA_SHIP_OVERRIDE_REASON="<причина>" + AURORA_SHIP_OVERRIDE_SCOPE=tree|push|ci — только осознанное точечное исключение, не обход по умолчанию',
    );
  }
}
...
export function checkDeliveryGuard(startDir = ROOT, { release = false } = {}) {
  ...
  if (!guard) {
    fail(
      'сторож доставки общего слоя не найден (aurora-meta/tools/check_crate_consumers.py)',
      'подтяните план-уровень: git -C <путь к aurora-meta> pull. '
        + 'Собирать без него нельзя: молчание сторожа неотличимо от его согласия',
    );
  }
  const guardArgs = [guard, '--crate', GATEWAY_MARK, '--tree', startDir];
  if (release) guardArgs.push('--release');
  const run = spawnSync('python', guardArgs, { cwd: startDir, encoding: 'utf8' });
  // 🔴 Код 9009 — «команды нет» на Windows, и приходит он БЕЗ `run.error`: заглушка магазина
  // приложений (`WindowsApps\python.exe`) — настоящий исполняемый файл, он запускается, ничего
  // не делает и выходит с этим кодом. Без отдельной ветки такой запуск попадал в разбор ниже и
  // сообщал «общий слой не доставлен: правки крейта не помечены» — ложный диагноз на пустом
  // месте, а ветка «нужен python в PATH» была для этого случая недостижима. 127 — то же самое
  // на *nix. Свои коды у сторожа — 0, 1, 2; спутать не с чем.
  const PYTHON_MISSING_CODES = [9009, 127];
  if (run.error || run.status === null || PYTHON_MISSING_CODES.includes(run.status)) {
    fail(
      `сторож доставки общего слоя не запустился: ${run.error?.message || `python вернул код ${run.status} — исполняемого файла нет`}`,
      'нужен python в PATH — проверьте командой python --version. На Windows код 9009 даёт '
        + 'заглушка магазина приложений: установите настоящий python либо снимите её в «Параметры → '
        + 'Приложения → Псевдонимы выполнения приложения»',
    );
  }
  // Вывод сторожа печатается ВСЕГДА, а не только при отказе: состояние общего
  // слоя — то самое, что человек обязан видеть перед выпуском.
  const text = `${run.stdout || ''}${run.stderr || ''}`.trimEnd();
  if (text) console.log(text);
  if (run.status === 2) {
    fail(
      'сторож доставки не смог измерить состояние общего слоя',
      'см. раздел ОТКАЗ выше: сторожу нечего мерить, и зелёного вывода тут быть не может',
    );
  }
  if (run.status !== 0) {
    fail(
      'общий слой не доставлен: правки крейта не помечены либо продукт отстал от метки',
      'см. раздел «НАРУШЕНИЯ, ВАЛЯЩИЕ ПРОГОН» выше. Цепочка общего слоя: слить в основную линию → '
        + 'поставить метку → обновить tag во фрагменте продукта → пересобрать. Линия продукта: '
        + 'свести рабочую ветку в основную либо отправить записи. Если задержка сознательная — '
        + 'подтвердите её переменной AURORA_CRATE_TAIL_ACK (хвост крейта) '
        + 'или AURORA_LINE_LAG_ACK (линия продукта) с причиной, она попадёт в вывод',
    );
  }
  info('сторож доставки общего слоя: нарушений в области этого дерева нет');
}
...
async function main() {
  ...
  // 🔴 Разбор режима поднят СЮДА, выше сторожа доставки: сторожу нужно знать,
  // порог это необратимости или нет. `--check` и `--test` никому ничего не
  // отправляют, а сборка установщика делает файл, который поедет людям, — и
  // только у неё отставание основной линии продукта обязано останавливать.
  // Разбор аргументов — чистое чтение, ничего за собой не оставляет.
  ...
  // Гоняется при КАЖДОМ запуске — сборке, --check и --test. Проверка, живущая
  // только в одном режиме, из настоящего выпуска выпадает.
