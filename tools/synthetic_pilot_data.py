...
GROUND_TRUTH_REAL_ESTATE = {
    ...
    # 2026-09-07 (замер): затухания были 0,80/0,55/0,60/0,25 — средний логит +0,22
    # против центра общего приора модели sigmoid(-1,4)≈0,20 (`engines/modeler.py:1019`).
    # NUTS уходил в воронку: 17 дивергенций на умолчаниях интерфейса
    # (4×2000+2000). Со сдвигом к приору — 0 дивергенций, прочие метрики не хуже.
    ...
    'y_std_frac': 0.20,
    # 🔴 2026-09-07 (замер на умолчаниях интерфейса, 4×2000+2000): было 0.05.
    # Шум ~1 % от целевой величины делал данные нереалистично чистыми: апостериор
    # наблюдательной сигмы прижимался к нулю, геометрия становилась воронкой,
    # NUTS давал 17 дивергенций (проверка сходимости у клиента — красная).
    # Ряд замеров: шум 0.05 → 17 дивергенций, 0.08 → 11, 0.12 → 1, 0.15 → 0
    # (ESS 2367 → 4758, BFMI 0.68 → 0.77, R² 0.855 → 0.847). Подбор затуханий
    # эту беду не лечил (0.55/0.35/0.45/0.20 давало 0 дивергенций, но ESS 215).
    'noise_frac': 0.15,
}
