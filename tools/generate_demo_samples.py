...
def generate_demo_fmcg(seed: int = 142) -> pd.DataFrame:
    ...
    Перекос: Наружная ~33% бюджета (ROI 0.8×), Performance недокормлен (ROI 4.2×).
    ...
    # Performance: умеренный объём кликов → ~10% бюджета (ROI 4.2×, НЕДОкормлен) — но
...
def generate_demo_otc(seed: int = 143) -> pd.DataFrame:
    ...
    Перекос: Аптечные материалы ~44% (ROI 0.7×), Детейлинг недокормлен
    (ROI 4.6×). «Детейлинг» — визиты медпредставителей (отраслевой термин). KPI «Продажи, упаковки». Контроли: Продажи категории,
    ...
    # Аптечные материалы: крупнейший бюджет (~44%, ROI 0.7×, перекормлен).
    # Детейлинг: НЕДОкормлен (~15%, ROI 4.6×), но достаточный объём/вариация
    # для уверенной OLS-оценки высокого ROI → оптимизатор наращивает его, давая lift.
