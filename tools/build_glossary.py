...
ID_MAP = {
    ...
    "Доверительный интервал (Posterior CI)": "posterior_ci",
...
LEGACY_ONLY = {
    "mmm": {
        ...
        "where": "Это основа всего продукта: пайплайн из 6 шагов от данных до оптимального бюджета.",
    ...
    "goal_seek": {
        ...
        "what": "Двойственная задача к прямой оптимизации. Aurora ищет минимальный бюджет, при котором прогноз достигает цели (функция отклика монотонна). Доверительный интервал на требуемый бюджет — через границы прогноза. Если цель за пределами безопасного коридора — «недостижимо».",
    ...
    "hierarchical_priors": {
        ...
        "what": "Aurora задаёт разные априорные ожидания по группам каналов. Бренд-каналы (ТВ, наружка, premium video) — длинный остаточный эффект; performance — короткий. Это улучшает различимость каналов на малых выборках.",
        "example": "ТВ ожидает 4–12 недель эффекта, performance — 1–3 недели. Если данные показывают обратное — модель сместится.",
...
<div class="footer">
  <p>Aurora AI Econometrica · © 2026 А. Сипович · глоссарий генерируется из единого источника</p>
</div>
