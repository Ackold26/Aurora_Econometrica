...
## v2.4.8 — устранена молчаливая потеря результата клиента (CPD-69, CPD-70) + снижение антивирусной поверхности (2026-08-13)

- **Папка выдачи больше не стирается при запуске (CPD-69):** очистка рабочей папки кабинета при старте программы задевала не только временные входящие файлы, но и уже готовые документы клиента (папка `exports`) — без предупреждения, без корзины. Перечень очищаемых каталогов сведён к одному только `inbox`.
- **Повторный экспорт больше не перезаписывает готовый документ молча (CPD-70):** сохранение отчёта (md/docx/xlsx/pptx/html) по вычисленному имени без проверки существования файла приводило к тихой потере прежнего результата при повторной генерации. Занятое имя теперь получает счётчик рядом («имя (2)», «имя (3)» и так далее), прежний файл не трогается.
- **Антивирусная поверхность снижена:** убран пакет, маскировавший публичный по конструкции ключ (ложный сигнал антивируса при нулевой пользе), мёртвый код в установщике, заполнены метаданные версии исполняемого файла.
- Версии 2.4.6/2.4.7 не выпускались клиентам — подвешены владельцем 12.08 из-за живого поведенческого детекта антивируса на той сборке (причина не в коде продукта, разбирается отдельно с вендором); выпуск продолжен под новым номером.

---

## v2.4.5 — канон честности MMM-методологии + починка облачного контура + единый источник вердикта надёжности + советник «Аврора» (2026-08-09)

Крупная консолидация: слияние долгоживущей линии `feat/econ-canon-p0` (девять блоков P0.1–P0.9, около семи недель работы) в основную ветку записью `49c09b2`. Между этой записью и предыдущей (v2.1.0, 2026-06-14) в этом файле образовался разрыв — версии 2.2.0–2.4.4 сюда не попали, хотя реально выходили (в них — переход на общий облачный контракт узла-шлюза, feature `thin`, локальная редакция collection). Восстанавливать их задним числом не стали (отдельная работа) — эта запись описывает то, что накопилось с 2026-06-20 по дату слияния.

- **Облачный совет починен (CPD-54):** подпись ответа входа проверялась некорректно — облачный режим (советник на базе ИИ) не отвечал ни одному клиенту с момента своего выпуска. Переход на `aurora_gateway v0.6.5` закрывает дефект.
- **Целостность доставки кабинетов (CPD-53):** обновление содержимого кабинетов (тексты советов, методология) стало атомарным — контрольная сумма файла проверяется до подтверждения записи, при сбое включается откат вместо частично осевшего обновления.
- **Канон честности MMM-методологии (P0.1–P0.9):** систематический пересмотр того, что расчёт говорит о себе. База окупаемости и пороги названы явно и берутся из одного источника по всему интерфейсу; доля пропусков в данных доезжает до экрана, а не замалчивается; воспроизводимость подтверждена зерном MCMC-сэмплера и паспортом среды расчёта; сертификат методологии не заверяет то, чего не проверял; убраны ложные обещания (готовая презентация «за 6 шагов», необъявленная метрика NRMSE); диаграмма чувствительности («торнадо») подключена к реальным данным вместо статики; отрицательный базовый уровень продаж и убыточный KPI больше не ломают проверку и не искажают её вывод; справка обращается на «вы».
- **Единый источник вердикта надёжности (Волна 1):** оценка надёжности модели больше не расходится между форматами — плашка надёжности выведена во все четыре отчёта (веб-страница, презентация, таблица, краткая сводка), причина ненадёжности доезжает до клиента тем же текстом, что и в интерфейсе.
- **Отчёты — фактические правки (Волна 2–3):** исправлены неверные значения в листе декомпозиции, периоде и числе наблюдений; глоссарий расширен с 11 до 47 терминов из единого источника; из клиентского текста убраны служебные имена провайдеров, англицизмы и случайные утечки внутренних данных в глоссарий.
- **Советник «Аврора» (Tier 2, новое):** ассистент поверх расчёта — поясняет результат простыми словами, предлагает сценарии «что если», отвечает в рамках методологии продукта. В интерфейсе — кнопка «Спросить Аврору»; из согласия на обработку убрано имя вендора модели, выбор модели в Оптимизаторе скрыт от пользователя.
- **Локальная редакция:** текст режима «только локально» приведён в соответствие с тем, что редакция реально делает (не обещает лишнего и не занижает то, что уже умеет); исправлен путь запуска расчётного модуля.
- **[заготовка — TODO team-lead]** Неделимость обновления файлов содержимого (гонка): выше уже описан первый проход (атомарная запись кабинетов, CPD-53) — если чинится что-то дополнительно, дописать сюда отдельным пунктом.
- **[заготовка — TODO team-lead]** Согласованность вердикта надёжности во всех форматах: выше уже описан первый проход (Волна 1) — если есть ещё правки, дописать сюда.
- **[заготовка — TODO team-lead]** Честный текст режима «только локально»: выше уже описана правка `1934e1a` — если в сборку войдёт что-то ещё по этой теме, дописать сюда.
